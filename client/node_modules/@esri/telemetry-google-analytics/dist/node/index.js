...
const telemetry_1 = require("@esri/telemetry");
/**
...
class GoogleAnalytics {
    ...
    getScriptTags() {
        return (0, telemetry_1.createScriptTags)(this._getScripts());
    }
    async init() {
        // for use in a broswer environment
        if (typeof window === 'undefined') {
            throw new telemetry_1.WindowUndefinedError('Window is undefined: Cannot add script element.');
        }
        ...
        await Promise.all(this._getScripts().map(scriptObj => {
            return (0, telemetry_1.injectScriptElementAsync)(scriptObj);
        }));
        // runs second script
        this.injectConfig();
    }
    ...
    logEvent(event) {
        ...
        let { action } = event, customParams = tslib_1.__rest(event, ["action"]);
