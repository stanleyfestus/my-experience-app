...
async function getNormalizer(mode) {
    ...
    const { normalize } = await Promise.resolve().then(function () { return require('./time-zones-d998b788.js'); });
    return normalize;
}
async function createTimeZoneItems(locale, messages, mode, referenceDate, standardTime) {
    if (mode === "name") {
        const { groupByName } = await Promise.resolve().then(function () { return require('./index-8f3d9d02.js'); });
    ...
    if (mode === "region") {
        const [{ groupByRegion }, { getCountry, global: globalLabel }] = await Promise.all([
            Promise.resolve().then(function () { return require('./index-18e29f38.js'); }),
            Promise.resolve().then(function () { return require('./region-0e49528b.js'); }),
        ]);
    ...
    const [{ groupByOffset }, { DateEngine }] = await Promise.all([
        Promise.resolve().then(function () { return require('./index-0d47af26.js'); }),
        Promise.resolve().then(function () { return require('./index-af3297c0.js'); }),
    ]);
...
const InputTimeZone = class {
    ...
    async componentWillLoad() {
        ...
        const [, normalizer] = await Promise.all([t9n.setUpMessages(this), getNormalizer(this.mode)]);
    ...
    renderItems() {
        ...
        return this.timeZoneItems.map((group) => {
            ...
            return (index.h("calcite-combobox-item", { "data-label": label, key: label, metadata: metadata, selected: selected, textLabel: label, value: value }));
        });
    }
    renderRegionItems() {
        return this.timeZoneItems.flatMap(({ label, items }) => (index.h("calcite-combobox-item-group", { key: label, label: label }, items.map((item) => {
            const selected = this.selectedTimeZoneItem === item;
            const { label, metadata, value } = item;
            return (index.h("calcite-combobox-item", { "data-label": label, description: metadata.country, key: label, metadata: metadata, selected: selected, textLabel: label, value: value }, index.h("span", { class: CSS.offset, slot: "content-end" }, metadata.offset)));
        }))));
    }
