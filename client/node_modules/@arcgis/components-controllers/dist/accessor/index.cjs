...
function trackPropKey(component, onResolved, defaultValue) {
  ...
  internals.keyTrackers.push((key, value) => (0, import_components_utils3.safeCall)(onResolved, void 0, defaultValue === value ? key : void 0));
  return defaultValue;
}
...
function trackPropertyKey(object, onResolved, defaultValue) {
  ...
  pendingTrackers.callbacks.push((keys2) => {
    const callback = (key2) => (0, import_components_utils4.safeCall)(onResolved, null, key2);
...
function watch(component, property, callback) {
  ...
  const safeCallback = (newValue, oldValue, propertyName) => (0, import_components_utils5.safeCall)(genericCallback, null, newValue, oldValue, propertyName);
...
var ControllerManager = class extends GenericController {
  constructor(component, forceUpdate) {
    ...
    this._updatePromise = new import_components_utils6.Deferred();
    ...
    if (integrations.size > 0) {
      for (const controller of integrations) {
        if ("triggerLoad" in controller) {
          ...
          await (0, import_components_utils6.safeAsyncCall)(controller.hostLoad, controller);
        }
      }
    }
  ...
  _setAutoDestroyTimeout() {
    ...
    if (!this.component.el.isConnected && !this.component.autoDestroyDisabled) {
      ...
      if (process.env.NODE_ENV !== "production" && autoDestroyOnDisconnectTimeout === 0) {
        ...
        this._autoDestroyTimeout = (0, import_components_utils7.devToolsAwareTimeout)(destroy, autoDestroyOnDisconnectTimeout);
      }
    }
  }
};
...
function callConnected(controller) {
  if ("triggerConnected" in controller) {
    ...
    (0, import_components_utils6.safeCall)(controller.hostConnected, controller);
  }
}
function callDisconnected(controller) {
  if ("triggerDisconnected" in controller) {
    ...
    (0, import_components_utils6.safeCall)(controller.hostDisconnected, controller);
  }
}
...
function callLoaded(controller) {
  if ("triggerLoaded" in controller) {
    ...
    (0, import_components_utils6.safeCall)(controller.hostLoaded, controller);
  }
}
function callUpdate2(controller) {
  if ("triggerUpdate" in controller) {
    ...
    (0, import_components_utils6.safeCall)(controller.hostUpdate, controller, this);
  }
}
function callUpdated(controller) {
  if ("triggerUpdated" in controller) {
    ...
    (0, import_components_utils6.safeCall)(controller.hostUpdated, controller, this);
  }
}
function callDestroy(controller) {
  if ("triggerDestroy" in controller) {
    ...
    (0, import_components_utils6.safeCall)(controller.hostDestroy, controller);
  }
}
...
var ReactiveUtilsIntegration = class extends Controller {
  async hostLoad() {
    ...
    bypassGetter(
      () => Object.keys(internals.members).forEach((name2) => {
        const alreadyIntegrated = internals.accessorSetter[name2] !== defaultGetterSetter;
        if (alreadyIntegrated) {
          return;
        }
        enabledMembers.push(name2);
        internals.accessorGetter[name2] = getter;
        internals.accessorSetter[name2] = setter;
        property({
          value: component[name2]
        })(AccessorSubclass.prototype, name2);
      })
    );
  ...
  controllerRemoved() {
    ...
    this._enabledMembers?.forEach((name2) => {
      internals.accessorGetter[name2] = defaultGetterSetter;
      internals.accessorSetter[name2] = defaultGetterSetter;
    });
  }
...
var accessorSupport = {
  ...
  reEmitAccessorEvents(controller, instance, reactiveUtils2, prefix) {
    ...
    const toListen = Object.entries(controller.component).map(([key, unknown]) => {
      ...
      const camelCaseEventName = (0, import_components_utils8.camelToKebab)(trimmedEventName);
      const eventName = camelCaseEventName.toLowerCase();
      return [eventName, emit];
    }).filter(import_components_utils8.isNotUndefined);
