...
var FunctionalController = class extends index$1.Controller {
  constructor(component, constructor) {
    super(component);
    const originalExports = this.exports;
    try {
      index$1.setAmbientComponent(this.component);
...
var proxy = index$1.proxyExports(FunctionalController);
...
var makeT9nController = (getAssetPath) => (options = {}) => (
  ...
  makeController((component, controller) => {
    ...
    if ("messageOverrides" in componentWithOverrides) {
      controller.onLifecycle(() => index$1.watch(componentWithOverrides, "messageOverrides", applyOverrides));
    }
    if (options.blocking) {
      return controller.ready;
    }
    const locale = index.getElementLocales(component.el);
    const pending = {
      _lang: locale.lang,
    ...
    () => controller.onLifecycle(
      () => toWatch.map(
        (name) => (
          ...
          index$1.watch(component, name, () => eventEmitter.emit({ name }))
        )
      )
    )
  );
