...
import type { NotifyOptions, BarAndLineTooltipFormatCallback, BarAndLineDataLabelFormatCallback, GuideTooltipFormatCallback, AxisLabelFormatCallback, MessageOptions } from "@arcgis/charts-js";
import type { ActionModes, SelectionData, ValidationStatus, SeriesSymbolsMap, SelectionTheme, ChartElementLimit, CursorRange, DataWarningObject } from "@arcgis/charts-shared-utils";
...
import type { CalculatedMinMaxBoundsPayload, LegendItemVisibility, WebChart, WebChartLegendPositions, WebMapWebChart, WebChartGenericDataItem, WebChartDataFilters, SupportedLayer } from "@arcgis/charts-spec";
import type { NoRenderPropChangePayload } from "../../interfaces";
import type { MissingSplitBySeriesProps } from "../../utils/interfaces";
/**
 * @slot action-bar - A slot for adding charts-action-bar component or calcite-action-bar.
 */
export declare class ArcGISChartsLineChart {
    /**
    ...
    private chart?;
    /**
     * The line chart HTML element.
     * @type {HTMLArcgisChartsLineChartElement}
     * @element
     */
    el: HTMLArcgisChartsLineChartElement;
    /**
    ...
    /**
     * When `true`, the series is completely hidden from the displayed chart if it doesn't have data (i.e. empty),
     * for example, after applying a data filter, filter by attribute or geometry (as when using the filter by extent)
     * @type {boolean | undefined}
     * @category Public
     * @default undefined
     */
    ...
    /**
     * Used to customize the number maximum of markers allowed on the chart.
     ...
     * If `addMissingSeries` is set to true new series will be added to cover the split-by values not referenced
     * by the config.
     * The default style from the properties `lineSymbol` and `markerSymbol` will be applied to the new series.
