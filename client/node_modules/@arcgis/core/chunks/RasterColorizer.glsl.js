...
}`),e.colorizerType===a.Stretch?C(o,e):e.colorizerType===a.Lut?b(o):e.colorizerType===a.Hillshade&&w(o,e),o}function b(e){e.fragment.main.add(g`vec2 pixelLocation = getPixelLocation(uv);
if (isOutside(pixelLocation)) {
fragColor = applyBackgroundBlend(vec4(0.0, 0.0, 0.0, 0.0));
return;
}
vec4 currentPixel = getPixel(pixelLocation);
fragColor = applyBackgroundBlend(colormap(currentPixel, true));`)}function C(e,a){e.fragment.uniforms.add(new d("u_bandCount",(e=>e.symbolizer.u_bandCount)),new f("u_minCutOff",(e=>e.symbolizer.u_minCutOff),3),new f("u_maxCutOff",(e=>e.symbolizer.u_maxCutOff),3),new f("u_factor",(e=>e.symbolizer.u_factor),3),new m("u_minOutput",(e=>e.symbolizer.u_minOutput)),new m("u_maxOutput",(e=>e.symbolizer.u_maxOutput)),new s("u_useGamma",(e=>e.symbolizer.u_useGamma)),new f("u_gamma",(e=>e.symbolizer.u_gamma),3),new f("u_gammaCorrection",(e=>e.symbolizer.u_gammaCorrection),3),new m("u_opacity",(e=>e.common.u_opacity))),e.fragment.code.add(g`float stretchOneValue(float val, float minCutOff, float maxCutOff, float minOutput, float maxOutput, float factor, bool useGamma, float gamma, float gammaCorrection) {
if (val >= maxCutOff) {
return maxOutput;
...
return stretchedVal;
}`);const l=a.applyColormap?g`fragColor = applyBackgroundBlend(colormap(vec4(grayVal, grayVal, grayVal, currentPixel.a), !u_useGamma));`:g`fragColor = applyBackgroundBlend(vec4(grayVal, grayVal, grayVal, currentPixel.a));`;e.fragment.main.add(g`
    ...
    vec4 currentPixel = getPixel(pixelLocation);
    ${a.stretchType===o.Noop?g`fragColor = applyBackgroundBlend(currentPixel);`:g`
    ...
    }`}`)}function w(e,a){const o=e.fragment;o.uniforms.add(new x("u_image",(e=>e.u_image)),new d("u_hillshadeType",(e=>e.symbolizer.u_hillshadeType)),new f("u_sinZcosAs",(e=>e.symbolizer.u_sinZcosAs),6),new f("u_sinZsinAs",(e=>e.symbolizer.u_sinZsinAs),6),new f("u_cosZs",(e=>e.symbolizer.u_cosZs),6),new f("u_weights",(e=>e.symbolizer.u_weights),6),new c("u_factor",(e=>e.symbolizer.u_factor)),new m("u_minValue",(e=>e.symbolizer.u_minValue)),new m("u_maxValue",(e=>e.symbolizer.u_maxValue)),new c("u_srcImageSize",(e=>e.common.u_srcImageSize))),o.include(n),o.code.add(g`vec4 overlay(float val, float minValue, float maxValue, float hillshade, float alpha) {
...
fragColor = applyBackgroundBlend(vec4(hillshade, hillshade, hillshade, alpha));`;o.main.add(g`
      vec2 pixelLocation = getPixelLocation(uv);
      if (isOutside(pixelLocation)) {
        fragColor = applyBackgroundBlend(vec4(0.0, 0.0, 0.0, 0.0));
        return;
      ...
      alpha *= u_opacity;
      ${l}`)}const z=Object.freeze(Object.defineProperty({__proto__:null,ColorizerHillshadeUniforms:y,ColorizerStretchUniforms:_,ColorizerUniforms:v,build:h},Symbol.toStringTag,{value:"Module"}));export{v as C,z as R,_ as a,y as b,h as c};
