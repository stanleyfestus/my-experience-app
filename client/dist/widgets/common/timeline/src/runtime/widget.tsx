...
import { getCalculatedTimeSettings, getDateTimePattern, getInsideLayersFromWebmap, getTimeExtentByTzOffset, getTimeSettingsFromHonoredWebMap, isSingleLayer } from '../utils/utils'
...
const Widget = (props: TimelineProps) => {
    ...
    if (dataSourceType === AllDataSourceTypes.WebMap) {
      if (useMapWidgetIds?.length) { // map widget
        Object.keys(layerViews).forEach(id => {
          ...
          updateLayerQueryParams(layer, queryParams, id)
        })
      } else { // webmap ds
  ...
  const onCreateDataSourceCreatedOrFailed = (dataSourceId: string, dataSource: DataSource, unMount = false) => {
    ...
    setDataSources(dataSources => {
      ...
      if (dataSources?.[dataSourceId] && !dataSource) { // current ds is removed by unselecting it.
        ...
        showWarning
          ...
          : !addSourceByData && dataSources === null // when set map without ds
              ...
              : <div
                ...
                >
                  {
                    !isOffPanel && <ReactResizeDetector handleWidth onResize={onResize} />
                  }
                  {
