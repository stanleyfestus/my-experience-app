...
function TimeLine (props: TimelineProps) {
  ...
  const timelineRulerByDIV = React.useMemo(() => {
    ...
    const seconds = getValueTickInfoForSecond(tsDetails, fullWidth, startTime, endTime, leftPosition, endPosition, scale)

    const tickSizes = getTickSizes(tsDetails, years, months, days, hours, minutes, seconds)
  ...
  const SpeedDropdown = React.useMemo(() => {
    return <Dropdown menuRole='listbox' activeIcon className={timeStyle === TimeStyle.Classic ? '' : 'justify-content-center'}>
              ...
              <div className='timeline-content-inside' >
                ...
                <div className='timeline-range-container' style={{ width: wMultiple * 100 + '%', marginLeft: -(leftPosition * wMultiple) + '%' }}>
                  <div className='resize-handlers' ref={el => { resizeRef.current = el }} style={{ marginLeft: startPositionForStep, width: isInstantMode ? 'fit-content' : widthForStep * 100 + '%' }}>
                    {
                      isInstantMode
                        ? <button className='resize-handler resize-instant' ref={ref => { instantRef.current = ref }} title={startLabelForStep} />
                        : <React.Fragment>
