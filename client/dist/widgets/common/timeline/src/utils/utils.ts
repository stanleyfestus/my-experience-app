import { type DataSource, dataSourceUtils, dateUtils, type FeatureLayerDataSource, type MapServiceDataSource, Immutable, DataSourceTypes, getAppStore, TimezoneConfig } from 'jimu-core'
...
/**
 * Get all inside featureLayers when current ds is webmap which support timeInfo.
 * @param dataSources
 * @returns
 */
...
export function isSingleLayer (dsType: string): boolean {
  return dsType === DataSourceTypes.FeatureLayer || dsType === DataSourceTypes.ImageryLayer || dsType === DataSourceTypes.ImageryTileLayer || dsType === DataSourceTypes.SubtypeGroupLayer
}
