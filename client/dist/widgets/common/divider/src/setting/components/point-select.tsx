  ...
  getLineStyles = (): SelectionOption[] => {
    ...
    for (const key in pointStyle) {
      ...
      const point =
        key === PointStyle.None
          ? { value: key }
          : { style: style, value: key as PointStyle }
    ...
    return (
      ...
      >
        <Select
          ...
          name='lineType'
          value={value}
          title={this.pointTitle[value]}
          onChange={this._onLineStyleChange.bind(this)}
          css={this.getSelectStyle()}
          aria-label={this.pointTitle[value]}
        >
          {this.getLineStyles().map((ls, index) => (
            <Option
              aria-label={this.pointTitle[ls.value]}
              role='option'
              tabIndex={index}
              key={index}
              value={ls.value}
              title={this.pointTitle[ls.value]}
              description={this.pointTitle[ls.value]}
            >
              <div className='w-100 pl-1 pr-1'>
                {ls.value === PointStyle.None && (
                  <div
                    className='position-relative'
                    css={this.getOptionStyle()}
                  >
                ...
                {ls.value !== PointStyle.None && (
                  <div
                    className='position-relative divider-line-con'
                    css={this.getOptionStyle()}
                  >
                    <div
                      className={classNames(
                        'position-absolute divider-line',
                        `${dividerLineClass}${ls.value}`
                      )}
                      css={this.getDividerLineStyle()}
                      style={{ border: '1px solid #fff' }}
                    />
                    <span className='position-absolute points' css={ls.style} />
                  </div>
                )}
              </div>
            </Option>
