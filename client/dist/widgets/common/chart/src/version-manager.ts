...
import { getSeriesType, WebChartTypes, type WebChartPieChartSeries, type WebChartPieChartLegend, type WebChartLineChartSeries } from 'jimu-ui/advanced/chart'
...
class VersionManager extends WidgetVersionManager {
  versions = [{
    ...
    upgrader: (oldConfig: IMConfig) => {
      ...
      const categoryType = getCategoryType(query)
      const statisticType = getStatisticsType(query)
      const hasTimeBinning = typeof (series[0] as ImmutableObject<WebChartLineChartSeries>).timeIntervalSize !== 'undefined'
