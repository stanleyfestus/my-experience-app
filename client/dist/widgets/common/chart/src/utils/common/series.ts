...
import { type CategoryFormatOptions, type DateTimeFormatOptions, getDefaultCategoryFormat, type WebChartLineChartSeries, getSplitByValue, WebChartSortOrderKinds, getSeriesType, type WebChartPieChartSeries, WebChartTimeIntervalUnits, getSplitByField, type WebChartBarChartSeries, type WebChartScatterPlotSeries, type WebChartHistogramSeries, WebChartTimeAggregationTypes, WebChartNullPolicyTypes, type WebChartGaugeSeries, type ChartTypes, type ISimpleLineSymbol, type ISimpleFillSymbol, WebChartTypes, type NumberFormatOptions } from 'jimu-ui/advanced/chart'
...
export const whetherUseIdFieldForNonCount = (query: ImmutableObject<FeatureLayerQueryParams>, idField: string) => {
  if (!idField || !query?.outStatistics?.length) return false
  const outStatistics = query.outStatistics
  const statisticsType = outStatistics[0].statisticType
  if (statisticsType === 'count') return false
...
export interface TimeBinningProps {
...
export const DefaultTimeBinningProps: TimeBinningProps = {
  timeIntervalSize: 1,
...
const getSeriesProps = (serie: ImmutableObject<WebChartSeries>, query: IMFeatureLayerQueryParams) => {
  ...
  const timeIntervalSize = (serie as unknown as WebChartLineChartSeries)?.timeIntervalSize
  const timeIntervalUnits = (serie as unknown as WebChartLineChartSeries)?.timeIntervalUnits
  const timeAggregationType = (serie as unknown as WebChartLineChartSeries)?.timeAggregationType
  const trimIncompleteTimeInterval = (serie as unknown as WebChartLineChartSeries)?.trimIncompleteTimeInterval
  const nullPolicy = (serie as unknown as WebChartLineChartSeries)?.nullPolicy

  const numericFields = outFields || outStatistics?.map((outStatistic) => outStatistic.onStatisticField)?.filter(field => !!field)

  return { splitByField, categoryField, numericFields, statisticType, binTemporalData, timeIntervalSize, timeIntervalUnits, timeAggregationType, trimIncompleteTimeInterval, nullPolicy }
}
...
export interface SplitBySerieProps {
  ...
  timeBinningProps: TimeBinningProps
}

export const createSplitBySerie = (props: SplitBySerieProps, index: number, colorProps?: SeriesColorProps) => {
  const { propSeries, name, categoryField, numberField, binTemporalData, timeBinningProps, splitByField, splitByFieldType, splitByValue } = props
  ...
  if (binTemporalData && timeBinningProps) {
    serie = {
      ...serie,
...
export const createSplitBySerieFromSeries = (
  ...
  const { splitByField, categoryField, numericFields, binTemporalData, timeIntervalUnits, timeIntervalSize, timeAggregationType, trimIncompleteTimeInterval, nullPolicy } =
    seriesProps
  ...
  const seriesValues = splitByValues.map((splitByValue, index) => {
    ...
    if (!serie) {
      let splitByProps = {
        ...
        splitByValue,
        binTemporalData,
        timeBinningProps: null
      }
      if (binTemporalData && timeIntervalUnits) {
        const timeBinningProps = {
          ...DefaultTimeBinningProps,
          timeIntervalSize,
          timeIntervalUnits,
          timeAggregationType,
...
export const createByGroupSeries = (props: SeriesProps, dataSourceId?: string): WebChartSeries[] => {
  const { splitByField, splitByFieldType, splitByFieldValues, categoryField, numericFields, statisticType, propSeries, binTemporalData, timeBinningProps } = props
  ...
  if (splitByField && splitByFieldValues.length) {
    ...
    const seriesValues = splitByFieldValues.map((splitByValue, index) => {
      const splitByProps = { propSeries, categoryField, numberField, binTemporalData, timeBinningProps, splitByField, splitByFieldType, splitByValue }
    ...
    const otherSplitByProps = { propSeries, name: SplitByOtherSeriesName, categoryField, numberField, binTemporalData, timeBinningProps, splitByField, splitByFieldType, splitByValue: SplitByOtherSeriesValue }
...
const getStatisticsTypeTranslation = (statisticsType) => {
  if (statisticsType === 'avg') {
    return 'mean'
  } else if (statisticsType === 'percentile_cont') {
    return 'median'
...
export const useDataSourceFeatureLayer = <T = any>(dataSourceId: string): T => {
  ...
  React.useEffect(() => {
    ...
    cancelable(dataSource.createJSAPILayerByDataSource()).then((layer: any) => {
...
function isComboLineAndBarChart (uniqueTypes: Set<string>): boolean | undefined {
  return (
    uniqueTypes.size === 2 &&
    uniqueTypes.has(WebChartTypes.LineSeries) &&
    uniqueTypes.has(WebChartTypes.BarSeries)
  )
}
...
function isBoxPlotChart (uniqueTypes: Set<string>): boolean {
  return (
    (uniqueTypes.size === 2 &&
      uniqueTypes.has(WebChartTypes.LineSeries) &&
      uniqueTypes.has(WebChartTypes.BoxPlotSeries)) ||
    (uniqueTypes.size === 1 && uniqueTypes.has(WebChartTypes.BoxPlotSeries))
  )
}
  ...
  if (chartSeries !== undefined && chartSeries.length > 0) {
    ...
    if (
      ...
      if (isBarLineCombo) {
        ...
        seriesType = WebChartTypes.BoxPlotSeries
...
export function getSeriesReference (
  ...
  let seriesReference =
    showMean
      ? series.find((s) => s.type === WebChartTypes.LineSeries) as unknown as ImmutableObject<WebChartSeries>
      : series[0]
