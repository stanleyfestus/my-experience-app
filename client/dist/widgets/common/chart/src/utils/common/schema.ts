...
const getSchemaForgetGaugeFields = (query: IMFeatureLayerQueryParams, dataSourceId: string) => {
...
const getIdSchema = (dataSource: DataSource) => {
  ...
  const idFieldSchema = dataSource.getDataSourceJson().schema.fields[idField]
...
export const getDataSourceSchema = (
  ...
  if (seriesType === 'histogramSeries') {
    ...
    schema = getSchemaForgetGaugeFields(query, originalDsId)
