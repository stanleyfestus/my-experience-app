...
import { getSeriesType, type WebMapWebChart, WebChartCurrentVersion, type WebChartSeriesType } from 'jimu-ui/advanced/chart'
 ...
 * Note: The chart stored in web map has the version, but there is no version on the chart of layer, so don't check it first.
...
const checkChartSpec = (WebMapWebChart: WebMapWebChart, dataSourceId?: string): SpecValidationStatus => {
  return composeCheckFn(checkSeriesType, checkSeriesField, checkSeriesWhere, checkSeriesQuery)(WebMapWebChart.series, dataSourceId)
}
