...
import { CollapsableToggle, defaultMessages as jimUiDefaultMessage, Select } from 'jimu-ui'
import { type ChartStatisticType, type ChartDataSource, type WebChartSeries, type WebChartOrderOptions } from '../../../../../../../config'
...
import { getSeriesType, WebChartNullPolicyTypes, type ChartTypes, type WebChartLineChartSeries, type WebChartTimeIntervalUnits, type WebChartTimeAggregationTypes, getSplitByField, type WebChartBarChartSeries, type CategoryFormatOptions, type NumberFormatOptions, type DateTimeFormatOptions } from 'jimu-ui/advanced/chart'
import { isSerialSeries } from '../../../../../../../utils/default'
import { createByGroupQuery, createByGroupSeries, getDefaultValueFormat, getSplitByFieldValues, getStatisticsType, getFieldType, getObjectIdField, DefaultTimeBinningProps } from '../../../../../../../utils/common'
...
export const ByGroupData = (props: ByGroupDataProps): React.ReactElement => {
  ...
  const propTimeBinningProps = timeIntervalUnits ? { timeIntervalSize, timeIntervalUnits, timeAggregationType, trimIncompleteTimeInterval, nullPolicy } : null
  const propsBinTemporalData = (propSeries?.[0] as unknown as WebChartLineChartSeries)?.binTemporalData ?? !!propTimeBinningProps
  ...
  const handleBinTemporalDataChange = async (binTemporalData: boolean): Promise<void> => {
    if (binTemporalData) {
      try {
        ...
        const nullPolicy = seriesType === 'barSeries' ? WebChartNullPolicyTypes.Null : WebChartNullPolicyTypes.Interpolate
        const timeBinningProps = {
          timeIntervalUnits,
          ...DefaultTimeBinningProps,
          nullPolicy
        }
        const series = createByGroupSeries({ splitByField, splitByFieldType, splitByFieldValues, categoryField, statisticType, numericFields, propSeries: propSeries.asMutable({ deep: true }), binTemporalData, timeBinningProps }, dataSourceId)
      ...
      const series = propSeries.map((serie) =>
        (serie as ImmutableObject<WebChartBarChartSeries>)
          .set('binTemporalData', binTemporalData)
          .without('timeIntervalSize')
          .without('timeIntervalUnits')
          .without('timeAggregationType')
          .without('trimIncompleteTimeInterval')
          .without('nullPolicy')
  ...
  const handleStatisticTypeChange = (statisticType: ChartStatisticType) => {
    ...
    const timeBinningProps = binTemporalData ? propTimeBinningProps : null
    const series = createByGroupSeries({ splitByField, splitByFieldType, splitByFieldValues, categoryField, statisticType, numericFields: _numericFields, propSeries: propSeries.asMutable({ deep: true }), binTemporalData, timeBinningProps }, dataSourceId)
  ...
  const handleNumericFieldsChange = (fields: string[]) => {
    ...
    const series = createByGroupSeries({ splitByField: _splitByField, splitByFieldType, splitByFieldValues, categoryField, statisticType, numericFields, propSeries: propSeries.asMutable({ deep: true }), binTemporalData: propsBinTemporalData, timeBinningProps: propTimeBinningProps }, dataSourceId)
  ...
  const handleSplitByFieldChange = (splitByField: string, values: Array<number | string>, splitByFieldType: JimuFieldType) => {
    const orderOptions = getDefaultOrderOptions(categoryField, !!splitByField)
    const series = createByGroupSeries({ splitByField, splitByFieldType, splitByFieldValues: values, categoryField, statisticType, numericFields, propSeries: propSeries.asMutable({ deep: true }), binTemporalData: propsBinTemporalData, timeBinningProps: propTimeBinningProps }, dataSourceId)
      ...
      </SettingRow>}
      {
        supportTemporalData && (
          <CollapsableToggle
            ...
            />
            {loadingDate && <Loading type='DONUT' />}
          </CollapsableToggle>
        )
      }
