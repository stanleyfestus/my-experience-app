...
const getSubmenuMode = (
  menuType: MenuType,
  subOpenMode: 'EXPAND' | 'FOLDABLE'
) => {
...
const upgradeOnePointOne = (oldConfig): IMConfig => {
  ...
  const variant = upgradeThemeNavVariant(
    _config?.main?.variants,
    _config.navType
  )
...
class VersionManager extends BaseVersionManager {
  versions = [
    ...
    },
    {
      ...
      upgrader: (oldConfig: IMConfig) => {
        if (oldConfig?.menuStyle !== 'pills' || !oldConfig?.advanced) return oldConfig

        let newConfig = oldConfig
        Object.keys(newConfig?.variant?.item || {}).forEach((key) => {
          if (newConfig.variant.item[key]?.borderRadius === '50rem') {
            newConfig = newConfig.setIn(['variant', 'item', key, 'borderRadius'], '6.25rem')
          }
        })
