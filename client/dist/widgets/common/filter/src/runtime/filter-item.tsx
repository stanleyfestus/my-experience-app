...
import {
  ...
  SqlExpressionMode, moduleLoader
} from 'jimu-core'
import { type filterItemConfig, FilterArrangeType, FilterItemType, FilterTriggerType } from '../config'
import { Switch, Icon, Button, Popper, Card, defaultMessages as jimuUIMessages, Alert, Select, Option } from 'jimu-ui'
...
export default class FilterItem extends React.PureComponent<Props, State> {
  ...
  loadSqlExpressionBuilder = () => {
    if (this.props.config.type === FilterItemType.Custom && !this.state.SqlExpressionBuilder) {
      moduleLoader.loadModule('jimu-ui/advanced/sql-expression-builder').then(modules => {
  ...
  render () {
    ...
    return (
      <div className={classNames('filter-item', { 'filter-item-custom': isCustomFilter })} role='group' aria-label={name}>
