/** @jsx jsx */
import { type ImmutableObject, jsx, hooks, classNames } from 'jimu-core'
...
import { type ControllerButtonStyle, type ControllerButtonStylesByState } from '../config'
...
export const SettingAdvanced = (props: AdvancedSettingProps) => {
  const { className, variant, defaultIsOpen = false, onChange, onReset } = props
            ...
            return (
              <Tab key={state} id={state} className="tab-title-item" title={translate(state === 'active' ? 'selected' : state)}>
                <SettingRow className='mt-4'>
                  <CollapsableResetPanel defaultIsOpen={defaultIsOpen} label={translate('text')} isEmpty={isEmpty(themeBoxStyles, textKeys)} bottomLine onReset={() => { handleReset(state, textKeys) }}>
                  <FontSetting
                ...
                <SettingRow>
                  <CollapsableResetPanel defaultIsOpen={defaultIsOpen} label={translate('advancedIconStyle')} isEmpty={isEmpty(themeBoxStyles, iconKeys)} bottomLine onReset={() => { handleReset(state, iconKeys) }}>
                ...
                <SettingRow>
                  <CollapsableResetPanel defaultIsOpen={defaultIsOpen} label={translate('shadow')} isEmpty={isShadowEmpty(themeBoxStyles.boxShadow)} onReset={() => { handleReset(state, ['boxShadow']) }}>
                    <BoxShadowSetting
