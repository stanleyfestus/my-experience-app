...
import { type ScrollListProps, type ListProps, ScrollList } from '../../common/scroll-list'
...
export interface LayoutListProps extends Omit<ListProps, 'lists' | 'createItem' | 'itemLength'>, Pick<ScrollListProps, 'syncScroll' | 'onScrollStatusChange'>, Pick<PopupListProps, 'advanced' | 'itemStyle'> {
  controllerId: string
  draggable?: boolean
...
export const LayoutList = React.forwardRef((props: LayoutListProps, ref: React.RefObject<HTMLDivElement>) => {
  const { controllerId, draggable, itemStyle, minLength, vertical, className, space, alignment, overflownStyle = OverflownStyle.Arrows, onItemClick, syncScroll, markerEnabled = true, autoSize, advanced, placeholder, onScrollStatusChange, onMouseDown } = props
      ...
      >
        ...
        {!placeholder && overflownStyle === OverflownStyle.Arrows &&
          <ScrollList
            ...
            autoScrollEnd={true}
            createItem={createItem}
            onScrollStatusChange={onScrollStatusChange}
            onMouseDown={onMouseDown}
          />
        }
        {placeholder}
