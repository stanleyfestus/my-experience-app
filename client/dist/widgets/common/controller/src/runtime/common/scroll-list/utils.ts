...
export const useResponsiveViewport = (props: ResponsiveViewportProps): ResponsiveViewportResult => {
  ...
  const number = Math.floor(length / itemLength) > counts ? counts : Math.floor(length / itemLength)
...
export const useDisplayButtonAmount = (props: ResponsiveViewportProps): number => {
  const { rootRef, lists, itemLength, autoSize, vertical, space, minLength } = props
  const counts = lists?.length ?? 0
  const length = useObserveDebouncedLength(rootRef, vertical, space, minLength, autoSize)
  const amount = Math.floor(length / itemLength) > counts ? counts : Math.floor(length / itemLength)
  return amount
}
