...
const useStyle = (vertical: boolean, space: number, minLength: number, alignment = ControllerAlignment.Center) => {
  return css`
    &.root {
...
export const PopupList = React.forwardRef((props: PopupListProps, ref: React.Ref<HTMLDivElement>) => {
  const { vertical, alignment, space, itemStyle, lists = [], createItem, itemLength, minLength = itemLength, autoSize, advanced, className, onMouseDown } = props

  const style = useStyle(vertical, space, minLength, alignment)

  const [rootRef, handleRef] = hooks.useForwardRef(ref)

  const displayButtonAmount = useDisplayButtonAmount({ rootRef, lists, itemLength, autoSize, vertical, space, minLength })
  ...
  return <div className={classNames('root popup-list-root', className)} css={style} ref={handleRef} onMouseDown={onMouseDown}>
    ...
    {hasMore &&
      <AvatarCard
        ref={moreButtonRef}
        className='popup-more-button popup-list-item'
