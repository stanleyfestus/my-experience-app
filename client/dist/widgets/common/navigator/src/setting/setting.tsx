...
import { type NavigationVariant, MultiSelect, Select, Radio, Label, Switch, TextInput, NumericInput, Tooltip, Button, defaultMessages as jimuiDefaultMessage, styleUtils, type NavIconButtonGroupVariant, type IconButtonStyles } from 'jimu-ui'
...
const Setting = (props: SettingProps) => {
  ...
  const { vertical, type, navStyle, variant, advanced, standard, paginationFontColor } = display
  ...
  const onAdvancedChange = () => {
    ...
    if (!advanced) {
      display = display.without('variant')
    }
  ...
  return <div className="widget-setting-navigator jimu-widget-setting" css={style}>
    <SettingSection>
      ...
      {section && <React.Fragment>
        ...
        {!isExpressMode && isTabStyle && <SettingRow flow='no-wrap' label={translate('showIcon')}>
          <Switch
        ...
        {!isExpressMode && isTabStyle && <SettingRow flow='no-wrap' label={translate('showText')}>
          <Switch
            aria-label={translate('showText')}
            checked={showText}
