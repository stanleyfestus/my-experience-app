...
import { type ViewNavigationType, type ViewNavigationDisplay, type IMViewNavigationDisplay, type ViewNavigationVariant } from './components/view-navigation'
import {
 ...
 * @param vertical
 */
export const useAdvanceStyle = (type: ViewNavigationType, navStyle: string, advanced: boolean, variant: ImmutableObject<ViewNavigationVariant>, vertical?: boolean, hideThumb?: boolean) => {
  return useMemo(() => {
    if (!advanced || !variant) return null
    const isRTL = getAppStore()?.getState()?.appContext?.isRTL
    if (type === 'nav') return navAdvanceStyle(navStyle, variant as ImmutableObject<NavigationVariant>, vertical)
...
const navAdvanceStyle = (navStyle: string, variant: ImmutableObject<NavigationVariant>, vertical?: boolean) => {
  return css`
    .jimu-nav{
...
const sliderAdvanceStyle = (variant: ImmutableObject<ThemeSliderVariant>, hideThumb: boolean, isRTL?: boolean) => {
  ...
  const trackBg = track?.bg || 'var(--ref-palette-neutral-600)'
  const thumbBg = thumb?.default?.bg || 'var(--ref-palette-white)'
  const thumbBorderColor = thumb?.default?.border?.color || 'var(--sys-color-primary-main)'
  const progressBg = progress?.default?.bg || 'var(--sys-color-primary-main)'
  const thumbStyle = `
 ...
 */
export const useNavTemplates = (): NavTemplate[] => {
...
export const useHandleViewsLayoutChangeInExpressMode = (section: string, getAppConfigAction: (appConfig?: IMAppConfig) => AppConfigAction) => {
  ...
  const viewsNeedUpdate = React.useMemo(() => {
    ...
    return sectionViews.filter((viewJson) => {
      ...
      if (largeLayoutHasWidget && mediumLayoutHasWidget && smallLayoutHasWidget && !viewJson.icon) {
        return true
      }
      return false
    })
  }, [allViews, isExpressMode, layouts, sectionViewIds])
  ...
  React.useEffect(() => {
    ...
    viewsNeedUpdate.forEach((viewJson) => {
      const { layout, id: viewId } = viewJson
