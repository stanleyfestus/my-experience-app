/** @jsx jsx */
import { React, jsx, css, polished, LayoutType, ReactRedux, type IMState, hooks, type ImmutableObject, type UtilitiesJson } from 'jimu-core'
...
const Setting = (props: SettingProps) => {
        ...
        <SettingRow flow='wrap' label={nls('printMode')} role='group' aria-label={nls('printMode')}>
          <div className='d-flex w-100 select-mode-con'>
            <div className='flex-grow-1 text-truncate'>
              <Button variant='text' className='w-100' disableHoverEffect={true} disableRipple={true} title={nls('printClassic')} active={config?.modeType === ModeType.Classic} onClick={() => { handleModeTypeChange(ModeType.Classic) }}>
                <Icon autoFlip icon={require('./assets/Classic.svg')}/>
              </Button>
              <div className='mt-2 w-100 text-center text-truncate' title={nls('printClassic')}>{nls('printClassic')}</div>
            </div>

            <div className='flex-grow-1 ml-2 text-truncate'>
              <Button variant='text' className='w-100' disableHoverEffect={true} disableRipple={true} active={config?.modeType === ModeType.Compact} title={nls('printCompact')} onClick={() => { handleModeTypeChange(ModeType.Compact) }}>
                <Icon autoFlip icon={require('./assets/Compact.svg')}/>
              </Button>
              <div className='mt-2 text-center text-truncate' title={nls('printCompact')}>{nls('printCompact')}</div>
            </div>
          </div>
        </SettingRow>
      </SettingSection>
  ...
  return (
    <div className='widget-setting-search jimu-widget-search' css={STYLE}>
      {/* Print source select */}
      <SettingSection className='map-selector-section' role='group' aria-label={nls('printSource')}>
