import { esri, portalUrlUtils, SessionManager, Immutable, UtilityManager, DataSourceManager, ArcGISDataSourceTypes, getAppStore, type ImmutableObject, type UseDataSource, type UseUtility, loadArcGISJSAPIModules, type AppRuntimeInfo } from 'jimu-core'
...
import {
  ...
  ReportTypes, LayoutTypes, DEFAULT_COMMON_SETTING, type ActiveItem
} from '../config'
...
async function getPrintServiceTaskUrl (printServiceUrl: string): Promise<TemplateInfoUrlInfo> {
  ...
  tasks?.forEach(taskName => {
    ...
    if (taskName?.includes('Report') || taskName?.includes('report')) {
...
function getAllLayers (layers, LegendLayer): any[] {
  ...
  layers?.forEach(layer => {
    ...
    // There is no current support for printing legend items for layers that are sent as a client-side image in the printout. https://developers.arcgis.com/javascript/latest/api-reference/esri-rest-print.html
    ...
    const isLayerUseBinning = layer?.featureReduction?.type === 'binning'
    const isLayerUsePieChartRenderer = layer?.renderer?.type === 'pie-chart'
    if (layer.type !== 'graphics' && layer.visible && layer?.legendEnabled && !isLayerHasEffect && !isLayerUseBinning && !isLayerUsePieChartRenderer) {
...
export const getDsRootIdsInConfig = (): ImmutableArray<string> => {
  ...
  Object.keys(dataSources).forEach(key => {
    ...
    if (ds?.type !== ArcGISDataSourceTypes.WebScene) {
      // is root ds
      rootIds.push(ds.id)
    }
  })
...
export const checkIsReportDsInUseMap = (reportItem, useMapWidgetIds?: ImmutableArray<string>): boolean => {
  if (!reportItem?.exbDatasource || reportItem?.exbDatasource?.length === 0 || !useMapWidgetIds) {
    return true
  } else {
    let isInMap = true
    ...
    if (param.name === 'Format') {
      ...
      if (param.defaultValue) {
        ...
        choiceList?.forEach(formatItem => {
          if (formatItem.include('pdf') || formatItem.include('png')) {
