import { React, type ImmutableArray, Immutable, DataSourceManager, utils as jimuCoreUtils, DataSourceTypes } from 'jimu-core'
import { type JimuMapView, JimuMapViewComponent, type JimuLayerView, LayerTypes } from 'jimu-arcgis'
...
export default function UseMapEntry (props: UseMapEntryProps) {
  ...
  const dataSourceItemRuntimeInfoMapRef = React.useRef<DataSourceItemRuntimeInfoMap>()
  dataSourceItemRuntimeInfoMapRef.current = dataSourceItemRuntimeInfoMap

  const clearAllDataSourceItemRuntimeInfoMapRef = React.useRef<ClearAllDataSourceItemRuntimeInfoMap>()
  clearAllDataSourceItemRuntimeInfoMapRef.current = clearAllDataSourceItemRuntimeInfoMap

  const removeNotUsedDataSourceItemRuntimeInfoMapRef = React.useRef<RemoveNotUsedDataSourceItemRuntimeInfoMap>()
  removeNotUsedDataSourceItemRuntimeInfoMapRef.current = removeNotUsedDataSourceItemRuntimeInfoMap

  const removeGeneratedJimuLayerViewDataSourceItemRuntimeInfoMapRef = React.useRef<RemoveGeneratedJimuLayerViewDataSourceItemRuntimeInfoMap>()
  ...
  const updateIMDataSourceItemsFromMapRef = React.useRef<() => void>()
  ...
  const createLayerDataSourcesForJimuMapViewAndUpdateIMDataSourceItemsFromMapRef = React.useRef<(_activeJimuMapView: JimuMapView) => void>()
  ...
  React.useEffect(() => {
    ...
    const jimuLayerViewCreatedListener = async (newJimuLayerView: JimuLayerView) => {
      ...
      // SubtypeGrouplayer and SubtypeSublayer are mutually exclusive. For generated data sources/layers, we only include SubtypeSublayers and exclude SubtypeGrouplayers.
      if (newJimuLayerView && isValidJimuQueriableLayerView(newJimuLayerView, allowMapBuiltinRef.current, allowGeneratedRef.current) && newJimuLayerView.type !== LayerTypes.SubtypeGroupLayer) {
    ...
    const jimuLayerViewRemovedListener = async (removedJimuLayerView: JimuLayerView) => {
