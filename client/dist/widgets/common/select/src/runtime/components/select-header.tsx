/** @jsx jsx */
import {
  React, jsx, css, hooks, type ImmutableArray, loadArcGISJSAPIModules,
  dataSourceUtils, classNames, DataSourceSelectionMode, getAppStore, appActions
} from 'jimu-core'
...
export default function SelectHeader (props: SelectHeaderProps) {
  ...
  React.useEffect(() => {
    if (needInteractiveAPIModules && !apiModules && !isLoadingApiModulesRef.current) {
      ...
      loadArcGISJSAPIModules([
        ...
        const [SketchViewModel, GraphicsLayer, jsonUtils, geometryEngine] = modules as [typeof __esri.SketchViewModel, typeof __esri.GraphicsLayer, typeof __esri.jsonUtils, typeof __esri.geometryEngine]
        setApiModules({
          SketchViewModel,
          GraphicsLayer,
          jsonUtils,
  ...
  const geometryEngine = apiModules?.geometryEngine
  ...
  const onSketchViewModelComplete = React.useCallback((apiView: __esri.MapView | __esri.SceneView, graphic: __esri.Graphic, selectionMode: DataSourceSelectionMode) => {
    ...
    try {
      if (geometry.type === 'point' || geometry.type === 'polyline') {
        ...
        if (tolerance > 0 && geometryEngine) {
          const bufferGeometry = geometryEngine.buffer(geometry, tolerance, 'meters')

          if (bufferGeometry) {
            geometry = bufferGeometry as __esri.Polygon
          }
        }
      }
    } catch (e) {
      console.error('geometryEngine.buffer() error of Select widget', e)
    }
    ...
    // console.log('onSketchViewModelComplete', 'selectionMode', selectionMode)
  ...
  }, [isSelectionModeEnabled, apiSpatialRel, availableRuntimeInfosForBatch, geometryEngine])
      ...
      <div className={classNames(['d-flex align-items-center dropdown-btn-container', { 'render-dropdown': shouldRenderDropDown }])}>
        {
          ...
          <Button
            ...
            type={isSelectedBtnActive ? 'primary' : 'default'}
...

        {
          ...
          >
            <DropdownButton
              ...
              type={ isDropdownOpened ? 'primary' : 'default' }
            />
            <DropdownMenu
              alignment="start"
              offset={[
