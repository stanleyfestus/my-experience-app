/** @jsx jsx */
import {
  React, jsx, css, hooks, DataSourceComponent, classNames, DataSourceSelectionMode,
  type DataSource, type ImmutableArray, DataSourceManager, dataSourceUtils, CONSTANTS, type ArcGISQueryParams
} from 'jimu-core'
import { defaultMessages as jimuUIMessages, Button, NumericInput, Select, Alert } from 'jimu-ui'
import { loadArcGISJSAPIModules } from 'jimu-arcgis'
import defaultMessages from '../translations/default'
import DataSourceExtension, { type DataSourceExtensionSourceType } from '../data-source-extension'
...
export default function SelectByLocation (props: SelectByLocationProps): React.ReactElement {
  ...
  const selectedDataSource = React.useMemo(() => {
    ...
    // TODO: need to check dataSource.getStatus() here ?
    return dataSource as DataSourceExtensionSourceType
  ...
  const onGetGeometrySuccessfully = React.useCallback((geometryInfo: GeometryInfo) => {
    ...
    appliedBatchRuntimeInfos.forEach(async runtimeInfo => {
  ...
  const selectByLocationFeatureCountTip = translate('selectByLocationFeatureCountTip', {
    count: featureCount
  })
...
async function getBufferedUnionGeometry (
  ...
  const queryParams: ArcGISQueryParams = {
    outFields: [],
    returnGeometry: true,
    // Must make sure the outSpatialReference is web mercator.
  ...
  const [geometryJsonUtils, geometryEngineAsync] = modules as [__esri.jsonUtils, __esri.geometryEngineAsync]

  // get raw geometries
  const rawGeometries: __esri.Geometry[] = []
  ...
  if (queryResult && queryResult.records.length > 0) {
    queryResult.records.forEach(record => {
      ...
      if (geometry) {
        if ((geometry as __esri.Geometry).declaredClass) {
          // geometry is Geometry instance
          rawGeometries.push(geometry as __esri.Geometry)
        } else {
  ...
  if (rawGeometries.length > 0) {
    if (enableBuffer && bufferDistanceInputValue > 0 && bufferUnitSelectValue) {
      ...
      // set unionResults to true to union the buffered geometries
      const unionResults = true

      // Must make sure the input geometry spatialReference is web mercator.
      // If the input geometry spatialReference is 4326, then geometryEngineAsync.buffer() will get the following error:
      // The input unit and the spatial reference unit are not of the same unit type.ie Linear vs.Angular
      // LinearUnit: "feet" | "kilometers" | "meters" | "miles" | "nautical-miles" | "yards"
      // AngularUnit: "degrees" | "radians"

      // bufferedUnionGeometry is a polygon
      const bufferResult = await geometryEngineAsync.buffer(rawGeometries, [bufferDistanceInputValue], apiBufferUnit, unionResults)
      ...
      if (rawGeometries.length === 1) {
        ...
        // union rawGeometries
        bufferedUnionGeometry = await geometryEngineAsync.union(rawGeometries)
      }
    }
  }
