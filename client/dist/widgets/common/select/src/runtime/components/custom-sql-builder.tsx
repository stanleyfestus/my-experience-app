/** @jsx jsx */
import { React, ReactDOM, jsx, css, hooks, moduleLoader, type IMSqlExpression, type DataSource, SqlExpressionMode, QueryScope } from 'jimu-core'
...
export default function CustomSqlBuilder (props: CustomSqlBuilderProps) {
  ...
  const [SqlExpressionBuilder, setSqlExpressionBuilder] = React.useState(null)
  ...
  React.useEffect(() => {
    async function loadSqlExpressionBuilder () {
      try {
        const modules = await moduleLoader.loadModules(['jimu-ui/advanced/sql-expression-builder'])
