/** @jsx jsx */
import {
  React, hooks, jsx, css, classNames, observeStore, DataSourceManager, DataSourceStatus, getAppStore, DataSourceSelectionMode,
  type IMDataSourceInfo, type DataRecord, type DataRecordSet, type IMSqlExpression, type FeatureLayerQueryParams, type DataSource, type SqlExpression
} from 'jimu-core'
...
export default function DataSourceListItem (props: DataSourceListItemProps): React.ReactElement {
  ...
  const dataRecordSet = React.useMemo((): DataRecordSet => {
    ...
    if (dataSource && selectedRecords.length > 0) {
      ...
      // dataRecordSet.fields = data source used fields + layer available fields
      let finalFields: string[] = [].concat(dsUsedFields).concat(layerFields)
  ...
  const isFinalNeedDataSource = React.useMemo(() => {
    ...
    const isSubtypeSublayer = jimuLayerView?.type === LayerTypes.SubtypeSublayer
  ...
  const onDataSourceInfoChangeRef = React.useRef<typeof onDataSourceInfoChange>()
  ...
  React.useEffect(() => {
    ...
    const dsExt = new DataSourceExtension(dataSource as DataSourceExtensionSourceType, widgetId)
  ...
  const tryExecuteSelectingByGeometryInfoAndSqlUIRef = React.useRef<typeof tryExecuteSelectingByGeometryInfoAndSqlUI>()
  ...
  const clearSelection = React.useCallback(async () => {
    ...
    const dsExt = new DataSourceExtension(ds as DataSourceExtensionSourceType, widgetId)
  ...
  const onCustomSqlBtnClicked = React.useCallback(() => {
  ...
  const style = React.useMemo(() => {
    // const borderTopStyle = isFilterPanelOpened ? 'border-top: 1px solid var(--primary-500);' : ''
          ...
          <div className='selection-count-container d-flex align-items-center pl-1'>
            {
              ...
              <div className='selection-count'>{selectedCount}</div>
            }
...

      {
        ...
        <div className={classNames(['filter-panel w-100', { 'd-none': !isFilterPanelOpened }])}>
          <div className='d-flex w-100 align-items-center mt-3'>
            <label className='w-100 pb-0 pr-1 select-sql-hint'>{finalSqlHint}</label>
