/** @jsx jsx */
import { React, jsx, css, defaultMessages as jimuCoreMessages, hooks, type IMThemeVariables, type SerializedStyles, i18n } from 'jimu-core'
...
export const TemplateEditor = (props: TemplateEditorProps): React.ReactElement => {
  const { editTemplate, selectedTemplates, featureLayerUrl, templates, onSaveTemplate, surveyItemInfo, onDeleteTemplate, theme, editDisabled } = props
  ...
  const saveTemplate = () => {
    ...
    if (id) {
      ...
      template.typeKeywords = [config.typeKeywords.survey123, config.typeKeywords.survey123Hub, config.typeKeywords.printTemplate, config.typeKeywords.featureReportTemplate]
    ...
    return uploadTemplate(template, allTemplates, surveyItemInfo, editDisabled, !isInfoDirty).then((tmpt) => {
      setIsSaving(false)
      setSaveError('')
  ...
  const checkSyntax = (fileobj?: any) => {
    ...
    return checkTemplateSyntax(fileobj || file, featureLayerUrl, surveyItemInfo).then((res) => {
      if (res.success) {
        ...
        const errorStr = translate('checkSyntaxErrMsg', { serverSideErrMsg: res.details && res.details[0] && res.details[0].description })
  ...
  const executeDeleteTemplate = () => {
    ...
    return deleteReportTemplate(template, surveyItemInfo).then((templateId) => {
      setIsDeleting(false)
      if (onDeleteTemplate) {
        ...
        <div css={css('position: relative;')}>
          {file && file.size
            ? <div className='w-100 file-name-box'>
                ...
                <Button className="p-0" icon={true} type={'tertiary'} onClick={() => { removeFile() }}>
        ...
        : <Fragment>
            <Button icon={true} type={'tertiary'} className='text-break check-syntax' onClick={() => { checkSyntax() }}>
              ...
              <span css={css('margin: 0 0.25rem;')}>
                {syntaxChecked && !syntaxError ? <SuccessOutlined size={16} color='var(--success)' css={css('margin: 0 6px;')}/> : ''}
                {syntaxError ? <WarningOutlined size={16} color='var(--warning)' css={css('margin: 0 6px;')}/> : ''}
              </span>
            </Button>
      ...
      ? <SettingRow flow='wrap' css={css('margin-top: 12px;')}>
          <Tooltip title={editDisabled ? translate('noActions') : ''} showArrow placement='left' interactive={true} leaveDelay={100}>
            <Button
              ...
              className="w-100 summary"
              disabled={surveyItemInfo.isCoowner || isDeleting || editDisabled}
