import { type React, type IntlShape } from 'jimu-core'
...
export function sliceUrlForSharing (isIncludeUrlParams: boolean): string {
  ...
  if (!isIncludeUrlParams) {
    let query = _keepUrlParams((window.location.search?.substring(1)), ['draft', 'org']) // remove: ?views={view}
    if (query) {
      query = '?' + query
    }
...
function _removePageInfoFromUrl (url) {
  const regex = /page\/[^/]+\/+/ // page info format: page/${pageId}/ or page/${pageLabel}/
