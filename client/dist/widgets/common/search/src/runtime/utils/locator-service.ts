...
export const getSuggestion = (geocodeItem: GeocodeListItem, searchText: string): Promise<Suggestion> => {
  return loadSuggest(geocodeItem?.geocodeURL, searchText, geocodeItem?.maxSuggestions, geocodeItem?.useUtility?.utilityId, geocodeItem?.countryCode).then(res => {
...
*/
export const loadGeocodeRecords = (address: string, maxResultNumber: number, geocodeItem: GeocodeListItem, searchResultView: SearchResultView) => {
...
export const loadGeocodeRecordsAndUpdateOutputDs = (option: updateOutputDsRecordsOptions) => {
  ...
  return addressToLocations(addressToLocationsOption).then(async (response) => {
    ...
    const graphics: __esri.GraphicProperties[] = newResponse?.map((res, index) => {
      ...
      extent = extent ? extent?.union(res.extent) : res.extent
      const graphic = {
...
export const addressToLocations = async (addressToLocationsOption: AddressToLocationsOption): Promise<__esri.AddressCandidate[]> => {
  const { geocodeItem, address, maxResultNumber, singleLineFieldName, addressFields, countryCode } = addressToLocationsOption
...
*/
export const loadSuggest = (geocodeURL: string, address: string, maxSuggestion: number, utilityId: string, countryCode: string): Promise<__esri.SuggestionResult[]> => {
