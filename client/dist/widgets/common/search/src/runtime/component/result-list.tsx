...
const ResultList = (props: ResultListProps) => {
  ...
  const openStateOfControllerWidget = ReactRedux.useSelector((state: IMState) => state.widgetsRuntimeInfo[id]?.state)
  ...
  useEffect(() => {
    openStateOfControllerWidget && setVersion(version + 1)
  }, [openStateOfControllerWidget])
  ...
  useEffect(() => {
    if (isOpenResultPopper || isToOtherWidget) {
      ...
      setDsHasAutoSelectFirstRecord(false)
  ...
  */
  ...
  const onSelectRecord = hooks.useEventCallback((option: SelectRecordsOption) => {
    ...
    MessageManager.getInstance().publishMessage(
      new DataRecordsSelectionChangeMessage(id, records, [datasourceId])
    )
