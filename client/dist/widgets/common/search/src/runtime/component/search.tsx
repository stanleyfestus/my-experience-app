...
import { type IMConfig, type Suggestion, SearchResultView, SourceType, type ServiceList, type NewDatasourceConfigItem, type InitResultServiceListOption, SearchServiceType, type IMServiceList, type IMSearchResult, type IMSelectionList, ArrangementStyle } from '../../config'
...
const SearchInput = (props: SearchSettingProps) => {
  ...
  const { config, className, reference, id, isShowSearchInput, isInCurrentView, isWidgetInCurrentPage, serviceList, searchResult, selectionList, datasourceConfig, loadingServiceList, showSearchSetting, jimuMapView, handleServiceListChange, clearSearchResult } = props
  ...
  */
  const querySuggestion = hooks.useEventCallback((starchText: string) => {
    ...
    const serviceSuggestion = getSuggestions(starchText, newServiceListRef?.current?.asMutable({ deep: true }), config, datasourceConfig as any)
  ...
  */
  const clearSearchValue = (isFocusInput = true) => {
    ...
    isFocusInput && searchInputRef.current?.focus()
  }
      ...
      <div className='h-100 flex-grow-1 search-input-con'>
        <div className='h-100 w-100' onKeyDown={inputConKeyup}>
          {isShowSearchInput && <TextInput
            ...
            className={classNames('h-100 w-100', { 'no-border-left': showSearchSetting })}
            suffix={suffix()}
            prefix={prefix()}
