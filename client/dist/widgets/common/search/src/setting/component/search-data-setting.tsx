...
const CustomSearchDataSetting = (props: Props) => {
  ...
  useEffect(() => {
    if (useMapWidgetIdsRef.current) {
      if (useMapWidgetIdsRef.current === useMapWidgetIds?.[0] && config.sourceType === SourceType.MapCentric) {
  ...
  */
  const updateDataConfigWhenHasDsAddedOrDeletedInMap = (jimuMapViews) => {
    const newConfig = config?.asMutable({ deep: true })
    const layerChangeRes = checkIsDsChangedInMap(jimuMapViews, config)
  ...
  const resetDataConfigOfMapCentric = async (views: { [viewId: string]: JimuMapView }, config: IMConfig) => {
  ...
  const restDataConfigOfCustomSearchSource = async () => {
      ...
      return (
        <div>
          {Object.keys(dataSourceConfigWithMapCentric).map(viewId => {
            ...
            return (
              <div className='mt-3' key={viewId}>
                <div className='d-flex align-items-center'>
                  {isWebScene ? <DataSceneOutlined className='mr-2' /> : <DataMapOutlined className='mr-2' />}
                  <SettingRow className='flex-grow-1' label={mapDataSourceLabel} aria-label={mapDataSourceLabel}>
                    <Button
  ...
  const renderSearchDataSettingItemWithMapCentric = (dataSourceConfigItemWithMapCentric: IMDataSourceConfigItemWithMapCentric, viewId: string) => {
    return (
      <div className='mt-2' key={viewId}>
        <SettingRow role='group' label={nls('customizeSearchSources')}>
        ...
        {showLoadingListRef.current?.[viewId] && <div className='position-relative mt-3 mb-3 loading-con'>
          <Loading type={LoadingType.Secondary} width={38} height={38}/>
        </div>}
      </div>
    )
  }
  ...
  return (
    <div css={STYLE} className={classNames({ 'con-border': config?.sourceType === SourceType?.MapCentric && !showPlaceholder })}>
      <SettingSection title={nls('source')}>
        ...
        <SettingRow>
          <div className='d-flex align-items-center ' onClick={() => { handleSearchSourceChange(SourceType.MapCentric) }}>
            <Radio checked={config?.sourceType === SourceType.MapCentric}/>
      ...
      {(!showPlaceholder && config?.sourceType === SourceType.MapCentric) && <SettingSection>
        <SettingRow label={nls('enableSearchFilter')}>
          <Switch
