...
export const StatusTips = React.memo((props: Props) => {
  ...
  return (
    <div className={'status-tips d-flex flex-column h-100 align-items-center justify-content-center'}>
      <div className={''}>
        {/* 1.no selected layer */}
        { (props.isNoSelectedLayersFlag && !props.isNoBuildingLayersFlag) && <WidgetBuildingExplorerOutlined size={40} color='var(--light-700)'/> }
        {/* 2.no building layer */}
        { props.isNoBuildingLayersFlag && <WarningOutlined size={40} color='var(--warning-900)'/> }
        {/* 3.no tool */}
        { props.isNoToolFlag && <InfoOutlined size={32} color='var(--info-500)'/> }
      </div>
