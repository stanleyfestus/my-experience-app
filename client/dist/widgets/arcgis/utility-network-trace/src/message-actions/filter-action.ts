...
import { type IMConfig } from './filter-action-setting'
...
export default class FilterAction extends AbstractMessageAction {
  ...
  onExecute (
    ...
    switch (message.type) {
      case MessageType.DataRecordsSelectionChange: {
        ...
        if (actionConfig) {
          if (
            actionConfig.messageUseDataSource &&
            actionConfig.actionUseDataSource
          ) {
            const records = (message as DataRecordsSelectionChangeMessage)
              .records
            if (
              !records ||
              records.length < 1 ||
            ...
            const messageDataSource = DataSourceManager.getInstance().getDataSource(
              actionConfig.messageUseDataSource.dataSourceId
            )
            const actionDataSource = DataSourceManager.getInstance().getDataSource(
            ...
            if (messageDataSource && actionDataSource) {
              ...
              if (actionConfig.enabledDataRelationShip) {
                ...
                if (
                  ...
                  const objectIdJimuFieldName =
                    messageDsSchema &&
                    messageDsSchema.fields &&
                    Object.keys(messageDsSchema.fields).find(
                      jimuFieldName =>
                        messageDsSchema.fields[jimuFieldName].esriType ===
                        'esriFieldTypeOID'
                    )
                  messageField =
                    messageDsSchema &&
                  ...
                  const messageJimuFieldName =
                    actionConfig.messageUseDataSource?.fields?.[0]
                  const actionJimuFieldName =
                    actionConfig.actionUseDataSource?.fields?.[0]
                  messageField = messageDataSource.getSchema()?.fields?.[
                    messageJimuFieldName
                  ]
                  actionField = actionDataSource.getSchema()?.fields?.[
                    actionJimuFieldName
                  ]
                }
                ...
                if (messageField && actionField) {
                  ...
                  for (let i = 0; i < tempMessage.records.length; i++) {
                    const tempFieldValue = tempMessage.records[i].getData()[
                      messageFieldName
                    ]
                    if (
                      messageFieldValues.includes(
                        `${this.formatValue(tempFieldValue, messageFieldType)}`
                      )
                    ) {
                      continue
                    } else {
                      messageFieldValues.push(
                        `${this.formatValue(
                          tempMessage.records[i].getData()[messageFieldName],
                          messageFieldType
                        )}`
                      )
                    }
                ...
                whereSql = this.getMoreConditionSql(
                  actionConfig,
                  actionDataSource,
                  whereSql
                )
                ...
                const whereSql = this.getMoreConditionSql(
                  actionConfig,
                  actionDataSource,
                  ''
                )
      ...
      case MessageType.ExtentChange: {
        if (!(message as ExtentChangeMessage).stationary) {
          // map is not static
          return
        }
        ...
        const extActionDataSource = DataSourceManager.getInstance().getDataSource(
          actionConfig?.actionUseDataSource?.dataSourceId
        )
        const extWhereSql = this.getMoreConditionSql(
          actionConfig,
          extActionDataSource,
          undefined
        )
  ...
  onRemoveListen (messageType: MessageType, messageWidgetId?: string) {
    ...
    if (
      this.lastMessage.type === messageType &&
  ...
  getRealQuerySql (whereSql: string, actionDataSource) {
    ...
    const realQuery = (actionDataSource as
      | FeatureLayerDataSource
      | FeatureQueryDataSource).getRealQueryParams(query, 'query')
