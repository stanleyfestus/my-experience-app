import { DataSourceManager, MessageManager, DataRecordsSelectionChangeMessage, type DataSourceJson, type ImmutableObject, type FeatureLayerQueryParams, getAppStore } from 'jimu-core'
import { DataSourceStatus, type FeatureLayerDataSource, type QueriableDataSource, type SubtypeSublayerDataSource } from 'jimu-core/data-source'
    ...
    if (queryObject) {
      if (queryObject?.data_version) {
        ...
        urlVerList.forEach((ver) => {
          ...
          if (this.unt) {
            this.unt.gdbVersion = verSplit[1]
            if (this.unt.utilityNetwork) {
