/** @jsx jsx */
import { type FeatureLayerDataSource, type JimuMapView, JimuMapViewComponent, MapViewManager } from 'jimu-arcgis'
import {
  type AllWidgetProps, type DataSource,
  DataSourceManager,
  type IMState,
  QueryScope,
  React,
...
import { DEFAULT_ACTIVATION, DEFAULT_ARRANGEMENT, HIGHLIGHT_LOCATION, MANUAL_PATHTRACING, SELECTED_FIELDS, SELECTED_LINE_FIELDS, SHOW_COMPASS_ORIENTATION, SHOW_LOCATION_ACCURACY, STREAMING, SYMBOL_COLOR, TIME_OUT, USE_MAPWIDGET, WATCH_LOCATION, ZOOM_SCALE } from '../constants'
...
export default class Widget extends React.PureComponent<WidgetProps, WidgetState> {
  ...
  handleTrackPath = () => {
    ...
    newPoint.LineID = getObjectId(this.pathDataSourceKey)
  ...
  updateTrackLine (track: TrackLinePoint, isCreate: boolean) {
    ...
    zoomToGraphics(this.state.jimuMapView, [getLineGraphic(lineObj.line, lineObj.tracks)], this.props.config.zoomScale ?? ZOOM_SCALE)
  }
    ...
    if (tracking) {
      ...
      const wId = watchPosition((position: GeolocationPosition) => {
        ...
        if (this.state.tempTracksWithLine) {
          ...
          track.LineID = getObjectId(this.pathDataSourceKey)
  ...
  onHandleLineDelete (args: { track: TrackLinePoint, line: TrackLine, type: string }) {
    ...
    if (type === 'point') {
      ...
      if (this.state.tempTracksWithLine && this.state.tempTracksWithLine.line && this.state.tempTracksWithLine.line.OBJECTID === track.LineID) {
        ...
        tracksWithLine.tracks = otherPoints.length === 1 ? tracks : otherPoints
        this.setState({
          operationRecords: tracksWithLine.tracks,
          locationOperation: Operations.DELETE,
          operation: otherPoints.length === 1 ? Operations.DELETE : Operations.UPDATE,
              ...
              <div className='header-section'>
                ...
                <div className='right'>
                  ...
                  <Tooltip title={visibleTitle} placement='bottom'>
                    <Button className='ml-auto' icon size='sm' type='tertiary' onClick={() => { this.handleLayerVisibleChange(Array.from(this.props.outputDataSources), !visibleIcon, 3) }} aria-label={visibleTitle}>
                  ...
                  {watchLocation && !this.state.tracking &&
                    (<Tooltip title={btnTitle} placement='bottom'>
                      <Button className='ml-auto' icon size='sm' type='tertiary' onClick={() => { this.handleTracking(true) }} aria-label={visibleTitle}>
                        <PlayOutlined />
                      </Button>
                    </Tooltip>)}
                  {watchLocation && this.state.tracking && (<Tooltip title={btnTitle} placement='bottom'>
                    <Button className='ml-auto' icon size='sm' type='tertiary' onClick={() => { this.handleTracking(false) }} aria-label={visibleTitle}>
                      <PauseOutlined />
                    </Button>
                  </Tooltip>)}
                </div>
             ...
             {manualPathTracing &&
              <Tooltip title={trackPathTitle} placement='bottom'>
                  <span className='btn-span' >
                    <Button className='btn-trace' disabled={!this.state.tracking} icon size='sm' type='tertiary' onClick={(e) => { e.stopPropagation(); if (this.state.tracking) { this.handleTrackPath() } }} aria-label={trackPathTitle}>
                        <TracePathOutlined />
                    </Button>
                  </span>
              </Tooltip>
