...
export default class OverviewMap extends BaseTool<BaseToolProps, States> {
  ...
  async tryCreateNewSmallView (): Promise<void> {
    ...
    // console.log('creating samllView')
  ...
  bindEventsToUpdateOverview () {
    ...
    this.watchMainViewBasemapChangeHandle = mainView.map.watch('basemap', () => {
      this.tryCreateNewSmallView()
    })

    this.watchMainViewExtentChangeHandle = mainView.watch('extent', () => {
      this.onMainViewExtentChange()
    })

    this.watchSmallViewExtentChangeHandle = smallView.watch('extent', () => {
  ...
  onBodyMouseUp = (evt: MouseEvent): void => {
    ...
    if (deltaX === 0 && deltaY === 0) {
      ...
      // calculate draged deltaMapX and deltaMapY
  ...
  /**
   * Update the position of overiew focus dom by this.tempFixedFocusExtent or mainJimuMapView extent
   */
  ...
  /**
   * Update the position of overiew focus dom by the input focusExtent
   * @param _focusExtent
   */
