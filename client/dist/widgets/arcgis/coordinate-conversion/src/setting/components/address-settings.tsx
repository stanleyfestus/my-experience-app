...
export default class AddressSettings extends React.PureComponent<Props, State> {
  ...
  constructor (props) {
    ...
    this.state = {
      ...
      displayFullAddress: this.props.config?.displayFullAddress ? this.props.config.displayFullAddress : false
    }
  }
  ...
  onDisplayFullAddressOptionChanged = (evt) => {
    this.setState({
      displayFullAddress: evt.target.checked
    }, () => {
      this.props.onAddressSettingsUpdated('displayFullAddress', this.state.displayFullAddress)
    })
  }
        ...
        <SettingRow>
          <Label check centric style={{ cursor: 'pointer' }} className='title3 hint-default'>
            <Checkbox role={'checkbox'} aria-label={this.nls('showFullAddress')}
              style={{ cursor: 'pointer' }} className='mr-2' checked={this.state.displayFullAddress}
