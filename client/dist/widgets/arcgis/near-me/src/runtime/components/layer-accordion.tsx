...
interface Props {
  ...
  index?: number
  onDownload?: (index: number, dsId: string, analysisId: string, analysisType?: string) => void
  dsId?: string
  analysisId: string
  analysisType?: string
...
export default class LayerAccordion extends React.PureComponent<Props, State> {
  ...
  onDownloadClick = (e) => {
    ...
    if (this.props.onDownload) {
      this.props.onDownload(this.props.index, this.props.dsId, this.props.analysisId, this.props.analysisType)
    }
  }
  ...
  render () {
    ...
    return (
      <div css={styles} style={{ border: this.props.isListView ? '' : '1px solid #d5d5d5' }} className={this.props.isListView ? 'layer-Container shadow py-1 w-100' : 'layer-Container py-0 w-100 card rounded-sm'}>
        <Row flow='wrap'>
