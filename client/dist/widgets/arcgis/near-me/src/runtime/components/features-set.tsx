...
export default class FeaturesSet extends React.PureComponent<Props, State> {
   ...
   */
  createFeatureItem = async (callFeatureDetailsClick: boolean, displayMapSymbol?: boolean) => {
    const featureRecord = this.props.selectedRecord as any
    ...
    individualFeatureItem = (
      <div>
        ...
        <div className='feature-widget-title-container nm-border-bottom-color'>
  ...
  renderSupportedActionList = () => {
    ...
    return (
      <React.Fragment>
        <Dropdown className={'float-right'} direction='down' size='sm' title={this.nls('actions')}
          useKeyUpEvent>
          <DropdownButton size='sm' arrow={false} css={actionButton} icon ref={this.dropdownRef}
            className='data-action-button' type='tertiary' aria-label={this.nls('actions')}>
  ...
  render () {
    ...
    return (
      <div style={{ borderTop: '1px solid #d5d5d5' }} className='feature-container w-100 m-0' css={featuresSetStyles}>
