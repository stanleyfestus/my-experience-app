...
export default class AnalysisSetting extends React.PureComponent<Props, State> {
  ...
  constructor (props) {
    ...
    if (this.props.activeDsLayersConfig) {
      this.state = {
        ...
        displayLayersResults: this.props.activeDsLayersConfig.displayAllLayersResult,
  ...
  displayAllLayersResultOptionChange = (evt: React.ChangeEvent<HTMLInputElement>) => {
    this.setState({
      displayLayersResults: evt.target.checked
    }, () => {
      this.props.onAnalysisSettingsUpdated('displayAllLayersResult', this.state.displayLayersResults)
    })
  }
  ...
  enableProximitySearchOptionChange = (evt: React.ChangeEvent<HTMLInputElement>) => {
    this.setState({
      enableProximitySearch: evt.target.checked
    }, () => {
      this.props.onAnalysisSettingsUpdated('enableProximitySearch', this.state.enableProximitySearch)
    })
  }
  ...
  */
  ...
  editOkButtonCLick = () => {
    this.setState({
      ...
      this.props.onAnalysisSettingsUpdated('layersInfo', this.state.newAddedLayerAnalysis)
      setTimeout(() => {
        this.props.getAddedLayersInfoUseDs()
      }, 100)
    })
    this.isAnalysisEdited = false
  }
      ...
      <div css={getAnalysisSettingStyle(this.props.theme, this.props.activeDsSearchConfig.searchByActiveMapArea)} style={{ height: '100%', width: '100%' }}>
        <div ref={this.analysisLayersPopperTrigger} className='mb-4'>
          ...
          {this.state.newAddedLayerAnalysis.length === 0 &&
            <SettingRow>
              <Alert tabIndex={0} style={{ minWidth: 'auto' }} withIcon={true} text={this.nls('noAnalysisConfiguredMsg')} type={'warning'} />
            </SettingRow>
          }
          ...
          <CollapsablePanel
            ...
            <div style={{ height: '100%', marginTop: 10 }}>
              ...
              {!this.props.activeDsSearchConfig.searchByActiveMapArea &&
                ...
                <SettingRow className={'ml-2'}>
                  <Label check centric className='w-100 labelStyle' style={{ cursor: 'pointer' }}>
                    <Checkbox role={'checkbox'} aria-label={this.nls('displayAllLayersResultLabel')}
                      style={{ cursor: 'pointer' }} className='mr-2' checked={this.state.displayLayersResults}
              ...
              {!this.props.activeDsSearchConfig.searchByActiveMapArea &&
                <SettingRow className={'ml-2'}>
                  <Label check centric className='w-100 labelStyle' style={{ cursor: 'pointer' }}>
                    <Checkbox role={'checkbox'} aria-label={this.nls('enableProximitySearchLabel')}
                      style={{ cursor: 'pointer' }} className='mr-2' checked={this.state.enableProximitySearch}
