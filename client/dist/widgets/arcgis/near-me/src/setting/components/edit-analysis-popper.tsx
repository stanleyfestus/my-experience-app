...
export default class EditAnalysisPopper extends React.PureComponent<Props, State> {
  supportedDsTypes = Immutable([AllDataSourceTypes.FeatureLayer, AllDataSourceTypes.SubtypeSublayer])
  items = []
      ...
      childDs.forEach((layer) => {
        if (layer?.type === DataSourceTypes.MapService || layer?.type === DataSourceTypes.GroupLayer || layer?.type === DataSourceTypes.SubtypeGroupLayer) {
          const recursiveCheckForGroupLayers = (grpLayer) => {
            ...
            grpChildlayers.forEach((subLayers) => {
              if (subLayers?.type === DataSourceTypes.MapService || subLayers?.type === DataSourceTypes.GroupLayer || subLayers?.type === DataSourceTypes.SubtypeGroupLayer) {
                ...
                if (subLayers?.layerDefinition?.geometryType) {
          ...
          if (dsRootIdsArr.length > 0) { //check for if map service child data source is same as feature layer ds id
            ...
            if (dsAdded && (layer as any)?.layerDefinition?.geometryType) dsRootIdsArr.push(layer.id)
          } else {
            if ((layer as any)?.layerDefinition?.geometryType) dsRootIdsArr.push(layer.id)
          }
        }
      })
    }
    ...
    return <div css={getAnalysisSettingStyle(this.props.theme, this.props.isActiveMapAreaSelected)} style={{ height: 'calc(100% - 112px)', width: '100%', overflow: 'auto' }}>
      ...
      <SettingSection>
        ...
        {this.props.isActiveMapAreaSelected && this.state.analysisType === AnalysisTypeName.Closest &&
          <Alert tabIndex={0} style={{ minWidth: 'auto' }} withIcon size='small' type='warning' className='w-100 shadow mb-2 mt-2'>
            ...
            <SettingRow>
              <Label className='w-100 d-flex'>
                <div className='flex-grow-1 text-break title3 hint-default'>
                  {this.nls('groupFeaturesLabel')}
                </div>
              </Label>
              <Tooltip role={'tooltip'} tabIndex={0} aria-label={this.nls('groupFeaturesInfoToolTip')}
                title={this.nls('groupFeaturesInfoToolTip')} showArrow placement='top'>
        ...
        {!this.props.isActiveMapAreaSelected && !this.state.summaryAnalysisType && this.state.selectedLayerGeometry === 'esriGeometryPolygon' &&
          <SettingRow label={this.nls('returnIntersectedPolygons')}>
            <Switch role={'switch'} aria-label={this.nls('returnIntersectedPolygons')} title={this.nls('returnIntersectedPolygons')}
              checked={this.state.returnIntersectedPolygons} onChange={this.returnIntersectedPolygonsChanged} />
          </SettingRow>
        }

        {/*Configure fields to export */}
        {(this.state.closestAnalysisType || this.state.proximityAnalysisType || this.state.summaryAnalysisType) &&
        ...
        {!this.props.isActiveMapAreaSelected && (this.state.closestAnalysisType || this.state.proximityAnalysisType) &&
          <SettingRow className='pt-1' label={this.nls('includeApproxDistance')}>
            <Switch role={'switch'} aria-label={this.nls('includeApproxDistance')} title={this.nls('includeApproxDistance')}
