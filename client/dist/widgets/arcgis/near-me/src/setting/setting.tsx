/** @jsx jsx */
import { React, jsx, getAppStore, classNames, Immutable, JimuMapViewStatus, type DataSource, DataSourceManager, DataSourceTypes, type DataSourceJson, type FieldSchema, type DataSourceSchema, JimuFieldType, EsriFieldType, type ImmutableArray } from 'jimu-core'
  ...
  updateConfigAsPerNewWebMap = async () => {
    ...
    if (this.props.useMapWidgetIds?.length > 0) {
      ...
      if (mapViewGroup?.jimuMapViews) {
        ...
        setTimeout(() => {
          this.saveUseDataSourcesProps(config)
        }, 200)
      }
    }
  }
 ...
 */
  ...
  */
   ...
   */
  saveUseDataSourcesProps = (config?) => {
    const outputDsJsons: DataSourceJson[] = this.getInitOutDataSource(config || this.props.config.configInfo)
    setTimeout(() => {
      this.props.onSettingChange({
        id: this.props.id,
        useDataSources: getUseDataSourcesForAllDs(config || this.props.config.configInfo)
      }, outputDsJsons)
    }, 200)
  }
