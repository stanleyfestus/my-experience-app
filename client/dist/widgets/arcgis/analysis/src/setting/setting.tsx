/** @jsx jsx */
import {
  React, jsx, css, type ImmutableObject, Immutable, hooks, type IMThemeVariables, ReactRedux, type IMState,
  i18n
} from 'jimu-core'
...
import { deleteToolsHasNoMatchedUtility, getRFxNameFromHistoryItem, isSameToolAsHistoryItem, outputParameterNeedAddToMapAuto, useAnalysisEnginesAccess, useGetDisplayedToolName, wait } from '../utils/util'
...
const Setting = (props: SettingProps) => {
  ...
  const supportedLocale = useMemo(() => i18n.getTranslatedLocale(locale, manifest.translatedLocales), [locale, manifest.translatedLocales])
  ...
  return (
    <div className='widget-setting-analysis jimu-widget-setting' css={style}>
      ...
      <SettingSection className={`${toolList.length === 0 ? 'no-border' : ''}`} title={translate('addTools')} role='group' aria-label={translate('addTools')}>
        ...
        <RFxToolSelector disabled={!useMapWidgetIds?.[0]} toolList={toolList} portal={portal} locale={supportedLocale} onWarningNoMap={() => { setOpenNoMapWarning(true) }} onChange={onAddNewRFx} />
