...
const CustomToolOptionConfig = (props: Props) => {
  ...
  return (
    <React.Fragment>
      <SettingRow className='label-dark-800 first-option' label={translate('showToolHelpLink')}>
        <Switch checked={option.showHelpLink} aria-label={translate('showToolHelpLink')} onChange={(e, checked) => { onChange(option.set('showHelpLink', checked)) }} />
      </SettingRow>
      ...
      <SettingRow className='label-dark-800 mt-3' label={
        ...
        <Select className='w-auto' value={option.messageLevel} defaultValue={MessageLevel.Warning} onChange={handleMessageLevelChange}>
