...
import { AnalysisToolParamDataType, type GPLinearUnit, type AnalysisToolParam, type AnalysisToolDataItem, type AnalysisServiceInfo } from '@arcgis/analysis-ui-schema'
...
import { type AnalysisValueTableCustomEvent, type AnalysisLinearUnitInputCustomEvent, type AnalysisNumberInputCustomEvent, type AnalysisStringInputCustomEvent } from '@arcgis/analysis-components'
...
const GPFeatureRecordSetLayerInputConfig = (props: GPTypeConfigProps) => {
  ...
  const { hideBrowseButton, enableSketch } = parameter as ImmutableObject<CustomToolParam>
...
const GenericMultiValue = (props: GPTypeConfigProps) => {
  ...
  return (
    <SettingRow className='mt-2 label-dark-400 last-setting-row' label={translate('defaultValue')} flow='wrap' role='group' aria-label={translate('defaultValue')}>
      <analysis-value-table ref={setValueTableRef} {...parameter} />
    </SettingRow>
  )
}
