...
const CustomToolOutputConfig = (props: Props) => {
  ...
  const needHideOutputParams = React.useMemo(() => getNeedHideOutputParams(parameters), [parameters])
      ...
      {parameters.map((parameter, index) => {
        ...
        return parameter.direction === 'esriGPParameterDirectionOutput' && !parameterNeedHide
          ? <CustomToolConfigCollapsablePanel key={parameter.name} parameter={parameter} onDisplayNameChange={(value) => { handleParameterChange(parameter.set('displayName', value), index) }}>
              ...
              {showAddResultLayersToMapAuto && <SettingRow className='mt-2 last-setting-row' label={translate('addResultLayersToMapAuto')} flow='no-wrap'>
