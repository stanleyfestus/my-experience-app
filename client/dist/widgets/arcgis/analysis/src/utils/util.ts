import { React, type ImmutableObject, MessageManager, DataSourcesChangeType, DataSourcesChangeMessage, type DataSource, DataSourceManager, SessionManager, ServiceManager, css, ReactRedux, type IMState, type UtilitiesJson, type ImmutableArray } from 'jimu-core'
import { MessageLevel, ToolType, type CustomToolConfig, type ToolConfig, type CustomToolOption } from '../config'

import { toCamelToolName, convertEsriMessageType, isEmptyDataItem, formatAnalysisEngineSuffix, canPerformStandardAnalysis, canPerformGeoAnalytics, canPerformRasterAnalysis, hasNAPrivilege, isStandardAnalysisServerAdvanced, type AnalysisHistoryItem, getRFxDefKeyFromRFT, type SerializedHistoryItem, isFeatureLayer, ErrorKeywords } from '@arcgis/analysis-shared-utils'
...
/**
 * TODO can delete this and import from analysis-shared-utils once they support custom tools and handle with 'string', 'number', 'boolean' type
 * Copy from @arcgis/analysis-shared-utils. because if message.description is number, JSON.parse will not throw error, but 'cost' in parsedMessage will and the error is not caught, must catch the error here.
...
export function getValidToolsList (activeAnalysisEngines: AnalysisEngine[], userHasNaPrivilege: boolean, toolsArray: AnalysisToolItem[], portal: __esri.Portal, hasAdvancedLevelLicense: boolean): AnalysisToolItem[] {
  const toolsWithAnalysisEngine = toolsArray.filter((tool: AnalysisToolItem) => {
    ...
    const isVisible = (isPortal && tool.visibilityMode === VisibilityModes.Enterprise) || (!isPortal && tool.visibilityMode === VisibilityModes.Online) || tool.visibilityMode === VisibilityModes.Both || isEmptyDataItem(tool.visibilityMode)
...
export function deleteToolsHasNoMatchedUtility (toolList: ImmutableArray<ToolConfig>, utilitiesState: ImmutableObject<UtilitiesJson>) {
  return toolList.filter((tool) => {
    ...
    // no utility info in tool config
    if (!utilityId) {
      return false
    }
    // has utility info in tool config but can't find utility
    if (!utilitiesState?.[utilityId]) {
      return false
    }
    // has utility info in tool config, can find utility, but url can't match
    const { url } = utilitiesState[utilityId]
...
export function customToolHasMapServiceLayerOutput (parameters: AnalysisToolParam[] | ImmutableArray<AnalysisToolParam>) {
  return !!parameters.find((p) => (p.dataType as any) === 'MapServiceLayer' && p.direction === AnalysisToolParamDirection.Output)
}
...
export function getNeedHideOutputParams (parameters: AnalysisToolParam[] | ImmutableArray<AnalysisToolParam>) {
