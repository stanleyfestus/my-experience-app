...
import { getLocaleInfo, isTravelModeParameter, isEmptyDataItem, StraightLineParameterValue, formatNumberToLocale, getPortalItemMemo, MaximumSignificantDigits, getServiceType, sanitizeUrl } from '@arcgis/analysis-shared-utils'
...
import { StringsManager } from '../../utils/strings-manager'
...
const useTempStrings = (type: ToolType, analysisEngine: AnalysisEngine) => {
  const [tempStrings, setTempStrings] = React.useState<{ [key: string]: string }>()
  React.useEffect(() => {
    const paths = ['arcgis-app-assets/i18n/arcgis-attributes.i18n', 'arcgis-analysis-assets/t9n/analysis-data-file-input.t9n']
    if (type !== ToolType.Custom) {
      paths.push('arcgis-analysis-assets/t9n/analysis-tool-standard-settings.t9n')
      if (analysisEngine === AnalysisEngine.Raster) {
        paths.push('arcgis-analysis-assets/t9n/analysis-tool-raster-settings.t9n')
      }
    }
    let strings = {} as { [key: string]: string }
    const locale = getLocaleInfo().locale
    Promise.all(paths.map((p) => StringsManager.getInstance().getStrings(`${urlUtils.getAbsoluteRootUrl()}arcgis-amd-packages/${p}.${locale}.json`).catch(() => ({})))).then((res) => {
      res.forEach((s: { [key: string]: string }) => {
        strings = { ...strings, ...s }
      })
      setTempStrings(strings)
    })
...
const HistoryParameters = (props: Props) => {
  ...
  const tempStrings = useTempStrings(type, analysisEngine)

  React.useEffect(() => {
    loadDataFiles().then(setDataFiles).catch((error) => {})
  }, [])
  ...
  const formatParameterObject = (
    ...
    if (parameter !== undefined) {
      ...
      if (!isEmptyDataItem(formattedParameter)) {
        if (dataType === AnalysisToolParamDataType.GPDataFile || dataType === AnalysisToolParamDataType.GPRasterDataLayer) {
          ...
          if (url !== undefined) {
            ...
            formattedFileParameter[tempStrings.item || 'Item'] = dataFiles.find((dataFile) => dataFile.portalItemID === dataFileParameter.portalItemID)?.name ?? dataFileParameter.portalItemID
          }
        ...
        if (dataType === 'GPComposite') {
          ...
          formattedParameter = {
            ...
            [tempStrings.dataType || 'Data type']: compositeDataType
  ...
  const translateRasterEnvironmentSettings = (
    ...
    translatedRasterESKeys.recycleProcessingWorkers = tempStrings.recycleProcessingWorkers || 'Recycle interval of processing workers'
    translatedRasterESKeys.parallelProcessingFactor = tempStrings.parallelProcessingFactor || 'Parallel processing factor'
    translatedRasterESKeys.retryOnFailures = tempStrings.retryOnFailures || 'Number of retries on failures'
    translatedRasterESKeys.processorType = tempStrings.processorType || 'Processor type'
    ...
    formattedRasterEnvironmentSettings.processorType = tempStrings.automatic || 'Automatic'
    ...
    if (isEmptyDataItem(context?.recycleProcessingWorkers)) {
      formattedRasterEnvironmentSettings.recycleProcessingWorkers = 0
    } else {
      formattedRasterEnvironmentSettings.recycleProcessingWorkers = context?.recycleProcessingWorkers
    }

    if (isEmptyDataItem(context?.parallelProcessingFactor)) {
      formattedRasterEnvironmentSettings.parallelProcessingFactor = translate('default')
    } else {
      formattedRasterEnvironmentSettings.parallelProcessingFactor = context?.parallelProcessingFactor
    }

    if (isEmptyDataItem(context?.retryOnFailures)) {
      formattedRasterEnvironmentSettings.retryOnFailures = 0
    } else {
      formattedRasterEnvironmentSettings.retryOnFailures = context?.retryOnFailures
    }

    if (isEmptyDataItem(context?.processorType)) {
      formattedRasterEnvironmentSettings.processorType = tempStrings.automatic || 'Automatic'
    } else {
      switch (context?.processorType) {
        case 'CPU':
          formattedRasterEnvironmentSettings.processorType = tempStrings.CPU || 'CPU'
          break
        case 'GPU':
          formattedRasterEnvironmentSettings.processorType = tempStrings.GPU || 'GPU'
          break
        default:
          formattedRasterEnvironmentSettings.processorType = tempStrings.automatic || 'Automatic'
      }
  ...
  const translateEnvironmentSettings = (jobParamsContext: AnalysisToolContext | undefined): {
    ...
    if (isEmptyDataItem(context)) {
      ...
      if (!isEmptyDataItem(context?.geographicTransformations)) {
        translatedESKeys.geographicTransformations = tempStrings.geographicTransformations || 'Geographic transformations'
  ...
  useEffect(() => {
    if (!parametersContainerNode || !combinedParameters || !toolT9n || !tempStrings) {
      return
    }
  ...
  // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [parametersContainerNode, toolT9n, translatedRFTParameters, tempStrings])

  useEffect(() => {
    if (!envSettingContainerNode || !combinedParameters || !toolT9n || !tempStrings) {
      return
    }
  ...
  // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [envSettingContainerNode, toolT9n, tempStrings])
