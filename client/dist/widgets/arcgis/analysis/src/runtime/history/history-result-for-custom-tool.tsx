...
const HistoryResultForCustomTool = (props: Props) => {
  ...
  const getValueTableDisplayedResult = (id: string, result: __esri.ParameterValue, outputOfCurrentResultItem: OutputConfig) => {
    ...
    return <div className='table-container'>
      <calcite-table bordered striped>
        {(value as Array<__esri.ParameterValue['value']>).map((v, index1) => {
          return (v as Array<__esri.ParameterValue['value']>).map((item, index2) => {
            ...
            return <calcite-table-row key={currentId}>
              <calcite-table-cell>{parameterInfo.displayName || parameterInfo.name || ''}</calcite-table-cell>
              <calcite-table-cell>{getDisplayResult(currentId, item, parameterInfo.dataType as __esri.ParameterValue['dataType'], outputOfCurrentResultItem)}</calcite-table-cell>
            </calcite-table-row>
          })
        })}
      </calcite-table>
