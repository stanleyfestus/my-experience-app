...
import { type AnalysisToolData, type AnalysisToolAppContainerCustomEvent } from '@arcgis/analysis-tool-app'
...
import { changeDisplayOfAnalysisToolButtons, useJobParamsFromOtherWidgets, useJobStatusChangeListener, usePortalHelpMap } from './utils'
...
const StandardTool = (props: ToolProps) => {
  ...
  useEffect(() => {
    const analysisToolElement = (analysisToolContainer as any)?.toolElement as HTMLAnalysisToolElement
  ...
  // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [(analysisToolContainer as any)?.toolElement])
  ...
  const { tempLayers, realJobParams } = useJobParamsFromOtherWidgets(toolData ? jobParamsFromOtherWidgets : null, jobParams, (analysisToolContainer as any)?.currentJobParams, mapLayers, toolData, updateJobParamsFromOtherWidgets)
