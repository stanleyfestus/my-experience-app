...
import { Popper, Button, Select } from 'jimu-ui'
import { type ToolConfig, type IMConfig, ToolType, type CustomToolConfig } from '../config'
import { type AnalysisToolUI, type AnalysisToolInfo, type AnalysisToolParam, type AnalysisToolUIParam, type LocaleItem, type FeatureCollection } from '@arcgis/analysis-ui-schema'
...
const useAnalysisSharedUtils = () => {
  ...
  React.useEffect(() => {
    moduleLoader.loadModule('arcgis-amd-packages/arcgis-analysis-shared-utils').then((res) => {
  ...
  return <div>
    ...
    {selectedTool && <div className='mt-4'>
      ...
      <div>
        ...
        {!validParameters.length && <div className='mt-2 warning-msg'><WarningOutlined className='mr-2' color='var(--warning-900)' />{translate('noApplicableParameter')}</div>}
      </div>
    </div>}
  </div>
}
...
export const SelectParameterPopper = (props: Props) => {
  ...
  hooks.useUpdateEffect(() => {
    ...
    if (selectedTool.type === ToolType.Standard) {
      ...
      const toolJsonPath = `${urlUtils.getAbsoluteRootUrl()}arcgis-amd-packages/arcgis-analysis-assets/assets/tool-json/${lowercaseToolName}.tool.json`
      const toolUIJsonPath = `${urlUtils.getAbsoluteRootUrl()}arcgis-amd-packages/arcgis-analysis-assets/assets/tool-ui-json/${lowercaseToolName}.tool.ui.json`
      const t9nPath = `${urlUtils.getAbsoluteRootUrl()}arcgis-amd-packages/arcgis-analysis-assets/assets/t9n/${lowercaseToolName}/${lowercaseToolName}.t9n.${locale}.json`
      Promise.all([
  ...
  const handleConfirm = async () => {
    ...
    if (isDsLevel && hasUrl && !isOutputFromWidget) {
      ...
      try {
        const featureSet = await featureUtils.convertDataRecordSetToFeatureSet({ dataSource, records, fields, name })
