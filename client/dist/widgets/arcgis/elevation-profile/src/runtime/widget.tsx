...
/** @jsx jsx */
import { React, type AllWidgetProps, BaseWidget, jsx, classNames, getAppStore, WidgetState, AppMode, type IMState, type DataRecord, DataSourceManager, DataSourceStatus, type FeatureLayerQueryParams, type QueriableDataSource, type DataSource, geometryUtils, DataSourceSelectionMode, type IMDataSourceInfo, UrlManager, urlUtils, DataSourceComponent, lodash, AllDataSourceTypes, type SubtypeGroupLayerDataSource, Immutable, type SubtypeSublayerDataSource, BrowserSizeMode } from 'jimu-core'
import {
...
import { type IMConfig, ButtonTriggerType, type AssetBufferIntersection, type LayerIntersectionInfo, type IntersectionResult, type ElevationLayersInfo, type NextPossibleOptions, type ProfileLayersSettings } from '../config'
import { JimuMapViewComponent, type JimuMapView, type FeatureLayerDataSource, type FeatureDataRecord, type JimuLayerView } from 'jimu-arcgis'
...
import { getAllLayersFromDataSource, defaultSelectedUnits, getPortalSelfElevationUnits, getPortalSelfLinearUnits, getRandomHexColor } from '../common/utils'
...
import { defaultElevationLayerSettings, defaultProfileSettings } from '../setting/constants'
...
interface IState {
  ...
  nextSelectableLineOptions: NextPossibleOptions
  profileResult: any
...
export default class Widget extends BaseWidget<AllWidgetProps<IMConfig> & ExtraProps, IState> {
  ...
  constructor (props) {
    ...
    this.state = {
      ...
      graphicsHighlightColor: activeDsConfig?.profileSettings?.graphicsHighlightColor,
      nextSelectableLineOptions: activeDsConfig?.profileSettings?.nextSelectableLineOptions,
      profileResult: null,
  ...
  getSelectedFeatureOnLoad = async () => {
    ...
    for (const dsLayerId in dsInfos) {
      ...
      if (layerDataSource?.type === AllDataSourceTypes.SubtypeGroupLayer) {
  ...
  onJimuLayerViewRemove = (jimuLayerView: JimuLayerView) => {
    if (this.isSupportAddedLayers()) {
      ...
      if (layerDs.type === AllDataSourceTypes.FeatureLayer || layerDs.type === AllDataSourceTypes.SubtypeSublayer) {
  ...
  setConfigForDataSources = () => {
    ...
    this.setState({
      ...
      graphicsHighlightColor: configActiveDs ? configActiveDs.profileSettings?.graphicsHighlightColor : this._defaultConfig.profileSettings.graphicsHighlightColor,
      nextSelectableLineOptions: configActiveDs ? configActiveDs.profileSettings?.nextSelectableLineOptions : this._defaultConfig.profileSettings.nextSelectableLineOptions,
      ...
      if (this._defaultViewModel.input?.geometry) {
        ...
        reactiveUtils.whenOnce(
          ...
          .then(async () => {
            ...
            if (this._defaultViewModel.chartData && isGroundProfileAvailable?.length) {
      ...
      if (defaultViewModel.input) {
        try {
          ...
          if (this._defaultViewModel.chartData) {
            ...
            const configuredGroundLayer = this.state.addedElevationLayers.filter((layerInfo) => layerInfo.id === this.state.groundLayerId)
            const isGroundProfileAvailable = this._defaultViewModel?.chartData?.lines?.filter((linesInfo) => linesInfo.id === configuredGroundLayer[0].id)
            if (!isGroundProfileAvailable?.length) {
              this.onErrorInChartData(true, this.nls('noProfileError'))
              return
            }
            //as it takes some time to update the chart stats, after adding this timeout then only we received the updated stats
  ...
  */
  ...
  getOutfieldsForDs = (dsLayer: FeatureLayerDataSource): string[] => {
    let outFields = []
    const layerDefinition = dsLayer.getLayerDefinition()
    ...
    //so to get the objectIdField always fetch it from the layerDefinition
    if (!outFields.includes(layerDefinition.objectIdField)) {
      outFields.push(layerDefinition.objectIdField)
    }
    return outFields
  }
  ...
  getFeatureFromLayer = async (dsLayerId: string, oid) => {
    ...
    //in case of newly added layer through other widgets we cannot get the layer in layer data source
    //so to get the objectIdField always fetch it from the layerDefinition
    query.where = dataSource.getLayerDefinition().objectIdField + ' = ' + oid
  ...
  componentDidUpdate = (prevProps) => {
    ...
    if (this.didElevationLayersColorLabelChanged(prevConfig?.elevationLayersSettings?.addedElevationLayers,
      ...
      prevConfig?.profileSettings.graphicsHighlightColor !== currentConfig?.profileSettings.graphicsHighlightColor
    ) {
      this.setState({
        graphicsHighlightColor: currentConfig?.profileSettings.graphicsHighlightColor,
        ...
        if (this._defaultViewModel) {
          //update the map indicator color for the ground layer
          const groundLayer = this.state.addedElevationLayers?.filter((layerInfo) => layerInfo.id === this.state.groundLayerId)
          if (this.state.groundLayerId === this._defaultViewModel.profiles.getItemAt(0)?.id && groundLayer[0].style.lineColor) {
            this._defaultViewModel.profiles.getItemAt(0)?.color?.setColor(groundLayer[0].style.lineColor)
          }

          //update the map indicator color when the map selection color changes
          if (this._mapView.view?.type === '3d' && this.state.graphicsHighlightColor) {
            this._defaultViewModel.profiles.getItemAt(this._defaultViewModel.profiles.length - 1)?.color?.setColor?.(this.state.graphicsHighlightColor)
          }
        }
      })
    }
    ...
    if (!lodash.isDeepEqual(prevConfig?.profileSettings?.nextSelectableLineOptions, currentConfig?.profileSettings?.nextSelectableLineOptions)) {
      this.setState({
        nextSelectableLineOptions: currentConfig?.profileSettings?.nextSelectableLineOptions
      }, () => {
        if (this.state.nextSelectableLineOptions?.displayNextSelectableLine) {
          if (this._nextPossibleSelectionLayer && this._nextPossibleSelectionLayer.graphics.length > 0) {
            const nextSelectableStyle = this.state.nextSelectableLineOptions.style
            const polylineSymbol = {
              type: 'simple-line',
  ...
  buildStatsValuesAsOutput = async (visibleProfiles, outputDs, selectedElevationUnit, selectedLinearUnit, isFlip) => {
  ...
  queryForNewSelection = async (featuresByLayer, selectedUsingEPSelectTool: boolean) => {
    ...
    if (Object.keys(featuresByLayer).length > 0) {
      for (const dsLayerId in featuresByLayer) {
        ...
        if (layerDataSource.type === AllDataSourceTypes.SubtypeGroupLayer) {
          //get the sublayer datasource by record id
          const subLayerDataSource = await (layerDataSource as SubtypeGroupLayerDataSource).getSublayerDataSourceByRecordId((layerDataSource as SubtypeGroupLayerDataSource).layerId)
          selectedLayerDsId = subLayerDataSource.id
  ...
  */
  ...
  filterExistingFeatures = (layer: FeatureLayerDataSource) => {
    ...
    //in case of newly added layer through other widgets we cannot get the layer in layer data source
    //so to get the objectIdField always fetch it from the layerDefinition
    const oIdField = layer.getLayerDefinition().objectIdField
      ...
      <div tabIndex={-1} className={'h-100 w-100 d-flex align-items-center mainSection'}>
        <div tabIndex={-1} className={'adjust-cards'}>
          <Card tabIndex={0} aria-label={this.nls('selectLinesDesc')} button data-testid='selectButton'
            className={classNames('front-cards mt-4 mb-4 shadow', this.state.updatedSelectableLayersAtRuntime.length > 0 && isProfileSettingsEnabled ? 'front-section' : 'hidden')}
          ...
          <Card tabIndex={0} aria-label={this.nls('drawProfileDesc')} button data-testid='drawButton'
            className={classNames('front-cards front-section mt-4 mb-4 shadow', this.state.updatedSelectableLayersAtRuntime.length === 0 || !isProfileSettingsEnabled ? 'h-100 ' : '')}
