 ...
 */
const getSizeInfo = (layer): { type: string } => {
  let sizeInfo = null
  if (layer.renderer.visualVariables) {
    layer.renderer.visualVariables.forEach((info: { type: string }) => {
...
const getPointSymbol = (graphic: __esri.Graphic, color: string): __esri.Graphic => {
  ...
  let symbol
  let isSymbolFound, graphicInfoValue, layerInfoValue, i, symbolShape,
...
*/
const updatePointSymbolProperties = (symbol, layerSymbol): __esri.SimpleMarkerSymbol => {
  let height: number
  let width: number
  let size: number
...
const getPolyLineSymbol = (graphic: __esri.Graphic, color: string): __esri.Graphic => {
  ...
  //if layer is valid and have valid renderer then only check for other symbol properties
  if (layer?.renderer) {
    if (layer.renderer.symbol?.hasOwnProperty('width')) {
    ...
    } else if (layer.renderer.defaultSymbol?.hasOwnProperty('width')) {
 ...
 */
export const getSketchSymbol = (geometryType: string, symbolColor?: string): __esri.Symbol => {
  let symbol
