...
import { React, jsx, type IntlShape, type IMThemeVariables, lodash, Immutable, type UseDataSource, DataSourceManager, type ElevationLayerDataSource, classNames } from 'jimu-core'
...
import { type ProfileStyle, type ElevationLayersInfo, type Statistics, type VolumetricObjOptions } from '../../config'
...
export default class ElevationLayerPopper extends React.PureComponent<Props, IState> {
  ...
  supportedDsTypes = Immutable([AllDataSourceTypes.ElevationLayer])
  constructor (props) {
    super(props)
  ...
  onElevationLayerSelect = (useDataSources: UseDataSource[]) => {
    ...
    if (layerDataSource.type === AllDataSourceTypes.ElevationLayer) {
      this.setState({
    ...
    return <div style={{ height: 'calc(100% - 92px)', width: '100%', overflow: 'auto' }} css={getElevationLayersSettingsStyle(this.props.theme)}>
      <SettingSection>
        ...
        <SettingRow className={classNames(!this.props.isVolumetricObjSettings ? 'ep-divider-top pt-4' : 'pt-1')} label={this.nls('label')} flow={'wrap'}>
          <TextInput className='w-100' role={'textbox'} aria-label={this.nls('label') + this.state.elevationLayerLabel} title={this.state.elevationLayerLabel}
        ...
        <SettingRow label={this.nls('displayGroundProfileStatsLabel')}>
