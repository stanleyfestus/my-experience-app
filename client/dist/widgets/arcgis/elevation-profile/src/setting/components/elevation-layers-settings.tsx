...
import { Label, Select, Option, Tooltip, Icon, defaultMessages as jimuUIDefaultMessages, Button, Checkbox } from 'jimu-ui'
...
import { type ElevationLayersInfo, type ElevationLayers, type Statistics, type VolumetricObjOptions } from '../../config'
...
export default class ElevationLayersSettings extends React.PureComponent<Props, IState> {
  ...
  constructor (props) {
    ...
    this.state = {
      ...
      groundLayer: this.props.config.groundLayerId,
    ...
    const layerItem = (
      <div className={classNames('flex-grow-1 layer-data-item cursor-pointer')} key={index}>
        <div className='d-flex w-100 justify-content-between align-items-center'>
          ...
          {item.id === this.state.groundLayer &&
            <Tooltip role={'tooltip'} tabIndex={0} aria-label={this.nls('groundLayerInfoTooltip')}
              title={this.nls('groundLayerInfoTooltip')} showArrow placement='top'>
  ...
  onGroundLayerChange = (evt: any) => {
    this.setState({
      groundLayer: evt.target.value
    }, () => {
      this.props.onElevationLayersSettingsUpdated('groundLayerId', this.state.groundLayer)
    })
  }
  ...
  render () {
    return <div style={{ height: '100%', width: '100%', marginTop: 5 }} css={getElevationLayersSettingsStyle(this.props.theme)}>
      ...
      <div ref={this.elevationLayerSidePopperTrigger} className='mb-4'>
        ...
        {this.state.addedElevationLayers.length > 0 &&
          <div tabIndex={-1} className={'w-100 mb-4 mt-4'}>
            <div className={'ep-layers-list'}>
              <List
                itemsJson={Array.from(this.state.addedElevationLayers).map((options: any, index) => ({
        ...
        <SettingRow>
          <Select menuRole={'menu'} aria-label={this.state.groundLayer} size={'sm'}
            name={'groundLayer'} value={this.state.groundLayer} onChange={this.onGroundLayerChange}>
        ...
        <div className='bg-default border-color-gray-400' css={getSidePanelStyle(this.props.theme)}>
          <SidepopperBackArrow
            ...
            <ElevationLayerPopper
              ...
              elevationLayersList={this.state.addedElevationLayers}
