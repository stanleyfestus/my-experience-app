...
import { type JimuMapView, MapViewManager, type FeatureLayerDataSource, loadArcGISJSAPIModules } from 'jimu-arcgis'
import { type StatisticsAttributes, type IMConfig, type Statistics, type ElevationLayersInfo, type VolumetricObjOptions } from '../config'
...
export default class Setting extends BaseWidgetSetting<AllWidgetSettingProps<IMConfig>, State> {
  ...
  private _availableLineLayers = []
  private _availablePointLayers = []
  mapView: JimuMapView
  ...
  constructor (props) {
    ...
    this.state = {
      ...
      dataSourceName: '',
      isElevationLayersSettingsOpen: false,
  ...
  checkForLayersAvailability = (activeDataSource: string) => {
    this._availableLineLayers = []
    this._availablePointLayers = []
    ...
    allLayerSources?.forEach((layer: FeatureLayerDataSource) => {
      if (layer.getLayerDefinition()?.geometryType === 'esriGeometryPolyline') {
        this._availableLineLayers.push(layer)
      }

      if (layer.getLayerDefinition()?.geometryType === 'esriGeometryPolyline' ||
          layer.getLayerDefinition()?.geometryType === 'esriGeometryPoint') {
        this._availablePointLayers.push(layer)
      }
    })
  }

  getConfigForSelectedDs = (activeDataSource: string, newConfig, groundLayer: boolean, allGroundLayers) => {
    ...
    if (!newConfig.configInfo.hasOwnProperty(activeDataSource)) {
      ...
      config.elevationLayersSettings.groundLayerId = elevationLayersConfig[0].id
  ...
  */
      ...
      waitForChildDataSourcesReady(this.mapView).finally(() => {
        ...
        for (const id in updatedMapViewGroups.jimuMapViews) {
          if (updatedMapViewGroups.jimuMapViews[id].dataSourceId) {
            ...
            this._groundData.push({
              dataSourceId: updatedMapViewGroups.jimuMapViews[id].dataSourceId,
              isGroundElevationLayerExists: isGroundLayer,
              groundElevationLayerUrl: allGroundLayers?.[0]?.groundElevationLayerUrl
            })
            newConfig = this.getConfigForSelectedDs(updatedMapViewGroups.jimuMapViews[id].dataSourceId, newConfig, isGroundLayer, allGroundLayers)
          ...
          if (updatedMapViewGroups && updatedMapViewGroups.jimuMapViews) {
            for (const id in updatedMapViewGroups.jimuMapViews) {
              ...
              this._groundData.push({
                ...
                isGroundElevationLayerExists: isGroundLayer,
                groundElevationLayerUrl: availableGroundLayers?.[0]?.groundElevationLayerUrl
              })
              this.setState({
                activeDataSource: 'default'
              })
              newConfig = this.getConfigForSelectedDs('default', newConfig, isGroundLayer, availableGroundLayers)
            }
          }
        }
  ...
  canAddDataSource = (dataSourceId: string): boolean => {
    ...
    this._dataSourceOptions.some((dsOption) => {
  ...
  */
  ...
  updateSelectableLayersStyle = (property: string, value: string | boolean, isNextSelectable: boolean) => {
    if (isNextSelectable) {
      this.props.onSettingChange({
        id: this.props.id,
        config: this.props.config.setIn(['configInfo', this.state.activeDataSource, 'profileSettings', 'nextSelectableLineOptions', property], value)
      })
    } else {
      this.props.onSettingChange({
        id: this.props.id,
                                  ...
                                  <CollapsablePanel
                                    ...
                                    <SettingRow flow='wrap'>
                                      <ElevationLayersSettings
                                        ...
                                        groundLayerInfo={this._groundData}
                                        isRTL={this.isRTL}
                                ...
                                <SettingSection>
                                  <React.Fragment>
                                    ...
                                    {this.props.config.configInfo[this.state.activeDataSource]?.profileSettings.isProfileSettingsEnabled &&
                                      <React.Fragment>
                                        {this._availableLineLayers.length === 0 &&
                                          <SettingRow>
                                ...
                                <SettingSection>
                                  <React.Fragment>
                                    ...
                                    {this.props.config.configInfo[this.state.activeDataSource]?.assetSettings?.isAssetSettingsEnabled &&
                                      this.state.showLayersSettings &&
                                      <React.Fragment>
                                        {this._availableLineLayers.length === 0 && this._availablePointLayers.length === 0 &&
                                          <SettingRow>
                                            <Alert tabIndex={0} className={'warningMsg py-1'}
                                              style={{ minWidth: 'auto' }}
                                              open={true}
