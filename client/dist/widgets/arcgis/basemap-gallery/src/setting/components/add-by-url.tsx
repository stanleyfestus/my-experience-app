/** @jsx jsx */
import { React, css, hooks, jsx, defaultMessages as jimuCoreMessages, uuidv1, urlUtils, lodash, ServiceManager } from 'jimu-core'
import { SettingRow } from 'jimu-ui/advanced/setting-components'
import defaultMessages from '../translations/default'
import { Button, type ImageParam, Loading, LoadingType, TextArea, TextInput, Tooltip, defaultMessages as jimuUIMessages } from 'jimu-ui'
import { type AllWidgetSettingProps } from 'jimu-for-builder'
import { type BasemapFromUrl, type IMConfig } from '../../config'
  ...
  return <div className='h-100 px-4 pb-4' css={style}>
    <div className='form-content'>
      <SettingRow label={<span>{translate('label')} <span style={{ color: 'var(--ref-palette-error-500)' }}>*</span></span>} flow='wrap' aria-label={translate('label')} role='group'>
        <TextInput className='w-100' required aria-required='true' type='text' value={title} placeholder={translate('basemapTitle')} onChange={(e) => { setTitle(e.target.value) }} />
      </SettingRow>
      <SettingRow
      ...
      <div className='img-container mt-3 mb-4'><img src={thumbnail?.url || require('../assets/default-thumbnail.svg')} /></div>
    ...
    <Button type='primary' aria-label={translate('save')} disabled={checkIfAddButtonDisabled()} onClick={onConfirm}>{translate('save')}</Button>
  </div>
}

const AddBasemapsByUrl = (props: AddBasemapsByUrlProps) => {
  const translate = hooks.useTranslation(defaultMessages, jimuUIMessages)
  return <ButtonWithSidePopper buttonText={translate('url')} sidePopperTitle={translate('addBasemapsByUrl')} buttonProps={{ 'aria-description': translate('urlTip') }}>
