...
const ImportBasemaps = (props: Props) => {
  const { portalUrl, portalSelf, config, onGroupBasemapItemsChange } = props
  ...
  const refreshBasemapItemsByGroupId = async (groupId: string, sortField?: SortField, sortOrder?: SortOrder) => {
    ...
    const newItems = await basemapUtils.getBasemapItemsByGroupId(portal, portalUrl, groupId, sortField, sortOrder, groupId === esriDefault3dGroupId, !groupInfo?.isUserGroup)
  ...
  return <ButtonWithSidePopper
    ...
    buttonProps={{ 'aria-description': translate('importTip'), style: { [isRTL ? 'marginLeft' : 'marginRight']: '8px' } }}
    ...
    <div className='h-100' css={sidePopperContentStyle}>
      <SettingSection className='pt-0 border-0' title={translate('chooseWebmaps')}>
        ...
        <SettingRow className='search-row'>
          ...
          />
          <Dropdown activeIcon menuItemCheckMode='multiCheck' className='ml-2'>
            <DropdownButton className='sort-btn' size="sm" title={translate('sort')} aria-label={translate('sort')} icon arrow={false}><SortOutlined /></DropdownButton>
            <DropdownMenu>
              <DropdownItem className='px-2' header style={{ color: 'var(--ref-palette-neutral-800)' }}>{translate('sortBy')}</DropdownItem>
              ...
              {showSortOrderPart && <React.Fragment>
                <DropdownItem divider className='mx-0' />
                <DropdownItem className='px-2' header style={{ color: 'var(--ref-palette-neutral-800)' }}>{translate('sortDirection')}</DropdownItem>
