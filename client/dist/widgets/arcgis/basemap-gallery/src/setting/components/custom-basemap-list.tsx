/** @jsx jsx */
import { React, jsx, type ImmutableObject, defaultMessages as jimuCoreMessages, hooks, css, Immutable } from 'jimu-core'
...
import { type IMConfig, type BasemapInfo, type BasemapFromUrl } from '../../config'
  ...
  return <React.Fragment>
    <List
      ...
      itemsJson={customBasemaps.asMutable().map((item, index) => {
        ...
        const thumbnailUrl = isBasemapFromUrl(mutableItem) ? mutableItem.thumbnail?.url : `${mutableItem.thumbnailUrl}?token=${token}`
        return {
      ...
      renderOverrideItemIcon={(actionData, refComponent) => {
        ...
        const onEdit = (e: React.MouseEvent<HTMLButtonElement, MouseEvent> | React.KeyboardEvent<HTMLButtonElement>) => {
