    ...
    return (
      <React.Fragment>{
        (isShowPointBtn && (flyStyle === FlyItemMode.Rotate || flyStyle === null)) &&
          <Button
            icon className='oper-btns btns draw-btn' type='tertiary' css={this.getBtnStyleForBuilder()}
            onClick={() => { this.onDrawBtnClick(DrawingMode.Point, { isTriggerCancel: true }) }}
            disabled={isDisable} title={pointTips} aria-label={pointTips}

            active={DrawingMode.Point === this.state.drawingMode}
            aria-pressed={DrawingMode.Point === this.state.drawingMode}
            variant={DrawingMode.Point === this.state.drawingMode ? 'contained' : 'text'}
            color={DrawingMode.Point === this.state.drawingMode ? 'primary' : 'default'}
          >
            <PinEsriOutlined />
          </Button>
      }{
        (isShowLineBtn && (flyStyle === FlyItemMode.Path || flyStyle === null)) &&
          <Button
            ...
            disabled={isDisable} title={lineTips} aria-label={pointTips}

            active={DrawingMode.Line === this.state.drawingMode}
            aria-pressed={DrawingMode.Line === this.state.drawingMode}
            variant={DrawingMode.Line === this.state.drawingMode ? 'contained' : 'text'}
            color={DrawingMode.Line === this.state.drawingMode ? 'primary' : 'default'}
          >
            <PolylineOutlined />
          </Button>
      }
  ...
  getClearBtnContent = (): React.ReactElement => {
    ...
    return (
      <Button icon onClick={this.handleClearBtnClick} disabled={isDisable}
        title={clearTips} aria-label={clearTips} className='btns clear-btn' type='tertiary'>
        <ClearOutlined />
      </Button>
    )
  }
