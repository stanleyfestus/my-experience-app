...
//TODO change AroundMapCenter icon
...
export default class InteractivePanel extends React.PureComponent<Props, State> {
  ...
  getRotateIconContent = (config: ItemsType): JSX.Element => {
    let icon = null
    ...
    if (isShowStyleSelector) {
      return (
        <Dropdown isOpen={this.state.isFlyStylePopupOpen} toggle={this.toggleFlyStylePopup} className='dropdowns flystyle-btn' activeIcon>
          <DropdownButton icon className='btns d-flex' title={styleTips} type='tertiary'
              arrow={isShowArrow} dot={isShowDot} disabled={isDisable}>
            {flyStyleContent}
          </DropdownButton>
  ...
  renderLiveviewSettingContent (): React.ReactElement {
    ...
    return (
      <Dropdown isOpen={this.state.isLiveView} toggle={this.toggleLiveviewSettingPopup} className='dropdowns liveview-btn'>
        <DropdownButton icon className='btns d-flex' disabled={isDisable} arrow={false}
          title={settings} aria-label={settings} type='tertiary'>
          <SettingOutlined />
        </DropdownButton>
  ...
  renderSpeedControllerContentPalette (): React.ReactElement {
    ...
    return (
      <Dropdown isOpen={this.state.isSpeedPopupOpen} toggle={this.toggleSpeedPopup} className='speed-controller-btn' direction='up'>
        <DropdownButton icon className='d-flex speed-controller-text' title={title} type='tertiary' arrow={false}>
          {formattedSpeed + 'x'}
        </DropdownButton>
