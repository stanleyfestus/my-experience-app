...
export const Slice = React.memo((props: Props) => {
  ...
  <React.Fragment>
    <SettingSection className='first-setting-section'>
      {/* tiltEnabled */}
      <SettingRow label={translate('tiltEnabled')} role='group' aria-label={translate('tiltEnabled')}>
        <Switch aria-label={translate('tiltEnabled')}
          checked={sliceConfig.tiltEnabled}
          onChange={evt => { _onSettingChanged({ tiltEnabled: evt.target.checked }) }}/>
      </SettingRow>

      {/* excludeGroundSurface */}
      <SettingRow label={translate('excludeGroundSurface')} role='group' aria-label={translate('excludeGroundSurface')}>
        <Switch aria-label={translate('excludeGroundSurface')}
