...
export default class FeatureInfos extends React.PureComponent<Props, State> {
  ...
  componentDidUpdate (prevProps: Props) {
    if (this.features) {
      ...
      const graphics = [this.props.graphic] || [graphic]
  ...
  updateGraphic (prevProps: Props, graphics) {
    // @ts-expect-error
    const popupTemplate = this.props.graphic?.popupTemplate || this.props.graphic?.layer?.popupTemplate
    if (!popupTemplate) {
      // @ts-expect-error
  ...
  createFeature () {
    ...
    return featureModulePromise.then(() => {
      ...
      this.features = new this.Features({
        container: this.refs.featureContainer,
        visible: true,
  ...
  render () {
    return (
      <div className='feature-info-component'>
        <div id='features-container' ref='featureContainer' />
      </div>
    )
  }
}
