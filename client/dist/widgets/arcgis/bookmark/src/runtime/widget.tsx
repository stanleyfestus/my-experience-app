...
> {
  ...
  showMapOriginLayer = (layersArray, visibleLayers) => {
  ...
  renderSlideViewTop = item => {
    ...
    return (
      <div
        className='w-100 h-100 bookmark-pointer surface-1 border-0 bookmark-slide-outline jimu-outline-inside'
  ...
  renderSlideViewText = item => {
    ...
    return (
      <div
        className='w-100 h-100 bookmark-pointer jimu-outline-inside surface-1 border-0'
        onClick={() => { this.onViewBookmark(item) }}
        key={item.id || `webmap-${item.name}`}
        role='listitem'
        tabIndex={0}
  ...
  renderCustomContents = item => {
    ...
    return (
      <div
        className='w-100 h-100 bookmark-custom-contents bookmark-pointer surface-1 border-0 jimu-outline-inside'
        onClick={() => { this.onViewBookmark(item) }}
        key={item.id}
        role='listitem'
        tabIndex={0}
  ...
  getMapBookmarks = (jimuMapView: JimuMapView) => {
    if (jimuMapView && jimuMapView?.dataSourceId) {
      ...
      let extraBookmarks = []
      if (view.type === '3d') {
        const mpSourceJson = mapSource.toJSON()
      ...
      switch (tempType) {
        ...
        case TemplateType.List:
          bookmarkExample = new Array(3).fill(1).map((item, index) => {
            return (
              <div className='d-flex bookmark-list-col surface-1 bookmark-pointer' key={index}>
      ...
      <ViewVisibilityContext.Consumer>
        {({ isInView, isInCurrentView }: ViewVisibilityContextProps) => {
          ...
          return (
            <Fragment>
              {embedLoad && (
                <div ref={this.rootRefCallback.bind(this)} className={classes} css={getStyle(theme, config, id, appMode, this.state.widgetRect, configLength, this.state.runtimeBmArray?.length, isWebMap)}>
                  <Fragment>
