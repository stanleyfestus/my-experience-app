...
export class MarkPopper extends React.PureComponent<Props, States> {
  ...
  checkChange (checkLayersConfig = false) {
    ...
    const mapViewIdChanged = this.isMapViewIdChanged(bookmark)
    const layersConfigChanged = checkLayersConfig && this.isLayersConfigChanged(bookmark)
    this.setState({ isChanged: graphicsChanged || extentViewpointChanged || mapViewIdChanged || layersConfigChanged })
  }
  ...
  isMapViewIdChanged (bookmark: Bookmark) {
    const { currentJimuMapView } = this.state
    if (!currentJimuMapView || !currentJimuMapView.view) return false
    return currentJimuMapView.id !== bookmark.mapViewId
  }
  ...
  addNewShowConfig = async () => {
    ...
    const newBookmark = (snapShot: ResourceItemInfo) => {
      ...
      const bookmark: Bookmark = {
        ...
        graphics: [],
        showFlag: true,
        mapViewId: currentJimuMapView?.id ? currentJimuMapView?.id : currentJimuMapView?.id,
  ...
  showDialogAndEdit = async () => {
    ...
    if (isOrgBookmarkInViewGroup && config && activeViewDsId && activeViewDsId !== config.mapDataSourceId) {
      viewGroup?.switchMap()
    }
    const view = currentJimuMapView.view
  ...
  handleJimuMapViewCreated = (jimuMapView: JimuMapView) => {
    ...
    const recursiveWatchLayer = (layers) => {
      layers.forEach(layer => {
        layer.watch('visible', (newValue, oldValue) => {
