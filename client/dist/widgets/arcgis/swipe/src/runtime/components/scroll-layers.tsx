...
import { type JimuMapView, type JimuLayerView, type JimuMapViewGroup } from 'jimu-arcgis'
...
export function ScrollLayers (props: ScrollLayersProps) {
  ...
  const swipesFirstRef = useRef<__esri.Swipe[]>([])
  ...
  const swipesSecondRef = useRef<__esri.Swipe[]>([])
      ...
      if (inactiveMapLayerViewsPromiseArray) {
        Promise.all(inactiveMapLayerViewsPromiseArray).then(jimuLayerViews => {
          ...
          swipesSecondRef.current = trailingArray?.map((layer) => {
            return new Swipe({
              id: widgetId,
              view: inactiveMapView?.view as __esri.MapView,
              disabled: true,
              position: 100,
        ...
        if (inactiveMapLayerViewsPromiseArray) {
          Promise.all(inactiveMapLayerViewsPromiseArray).then(jimuLayerViews => {
            if (swipeDirection === 'vertical') {
              inactiveMapView?.view?.watch('height', updateSizeSecond)
            } else {
              inactiveMapView?.view?.watch('width', updateSizeSecond)
            }
            updateSizeSecond()
        ...
        if (!tickingSecond.current) {
          requestAnimationFrame(() => {
            ...
            swipesSecondRef?.current?.forEach((swipe, index, swipes) => {
              ...
              swipe.when(() => {
                const dividers = multiMapContainerRef.current.querySelectorAll<HTMLElement>('.esri-swipe__divider')
    ...
    function updateSwipeWidgetFirst () {
      ...
      if (activeMapLayerViewsPromiseArray) {
        Promise.all(activeMapLayerViewsPromiseArray).then(jimuLayerViews => {
          ...
          if (swipeDirection === 'vertical') {
            view.watch('height', updateSizeFirst)
          } else {
            view.watch('width', updateSizeFirst)
          }
          updateSizeFirst()
