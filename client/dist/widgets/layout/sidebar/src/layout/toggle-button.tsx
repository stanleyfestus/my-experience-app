...
export class SidebarController extends React.PureComponent<SidebarControllerProps> {
  ...
  toggleVisible = (e) => {
    ...
    moduleLoader.loadModule('jimu-for-builder').then((module) => {
        ...
        <div ref={(elem) => { this.ref = elem }} css={this.getStyle(isExpressMode)} onMouseLeave={() => { this.ref.classList.remove('active') }}>
          ...
          {isExpressMode && this.isMainSizeMode() && (
            <ThemeSwitchComponent useTheme2={window.jimuConfig.isInBuilder}>
              <div className='toggle-visible-btn rounded-circle' title={i18n.getIntl().formatMessage({ id: showAsExpressTip ? 'show' : 'hide' })} onClick={this.toggleVisible}>
                {showAsExpressTip ? <InvisibleOutlined color='var(--ref-palette-black)'/> : <VisibleOutlined color='var(--ref-palette-black)'/>}
              </div>
