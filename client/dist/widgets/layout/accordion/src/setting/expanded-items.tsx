...
import { MultiSelect, Select } from 'jimu-ui'

export function ExpandedItems (props: { widgetId: string }) {
  ...
  const handleValuesChange = React.useCallback((e, v, values) => {
  ...
  return (
    isSingleMode
      ? <Select size='sm' css={css`.jimu-dropdown { width: 100%; }`} placeholder={translate('none')} value={values[0]} onChange={handleValueChange}>
        {allOptions.map((item) => <option key={item.value} value={item.value}>{item.label}</option>)}
      </Select>
      : <MultiSelect size='sm' css={css`.jimu-dropdown { width: 100%; }`} placeholder={translate('none')} values={values} items={allOptions} onClickItem={handleValuesChange} />
  )
}
