          ...
          .then((result: any) => {
            ...
            if (type === 'esriLRSNetworkLayer' ||
              ...
              if (isNetwork) {
                ...
                networkInfo.mTolerance = result.spatialReferenceInfo.mTolerance
                networkInfo.xyTolerance = result.spatialReferenceInfo.xyTolerance

                const eventsList = []
                const resultEventLayers = result.eventLayers
                if (isDefined(resultEventLayers) && resultEventLayers.length > 0) {
                ...
                if (resultIntersectionLayers && resultIntersectionLayers.length > 0) {
                ...
                const dataSource: any = originDs as DataSource
                let shapeLengthFieldName
                if (dataSource?.layerDefinition?.hasGeometryProperties &&
                  dataSource?.layerDefinition?.geometryProperties?.shapeLengthFieldName) {
                  shapeLengthFieldName = dataSource?.layerDefinition?.geometryProperties?.shapeLengthFieldName
                  excludedFields.push(shapeLengthFieldName)
                }
