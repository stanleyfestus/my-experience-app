...
export function RouteAndMeasureForm (props: RouteAndMeasureFormProps) {
  ...
  const [isMeasurePickerActive, setIsMeasurePickerActive] = React.useState<boolean>(false)
  ...
  */
  ...
  React.useEffect(() => {
    if (revalidateRouteFromDataAction) {
      ...
      if (!isNaN(routeInfoFromDataAction.selectedMeasure)) {
        setMeasureInput(routeInfoFromDataAction.selectedMeasure.toString())
      }
  ...
  React.useEffect(() => {
    if (isDefined(network)) {
      ...
      setIsMeasurePickerActive(false)
      clearPickedGraphic()
    }
  // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [network])
  ...
  const validateRouteIdOrName = (routeIdOrName: string, fromDataAction = false): Promise<ValidityResult> => {
    return new Promise(resolve => {
      setTimeout(() => {
        ...
        const isValid = queryRouteIdOrName(routeIdOrName.trim(), network, networkDS)
          .then(async (results) => {
  ...
  const validateMeasure = async (value: string): Promise<ValidityResult> => {
    ...
    if (!isNaN(measure)) {
      if (isDefined(routeInfo.selectedPolyline)) {
        ...
        if (isDefined(routeInfo.selectedPoint) && !isNaN(routeInfo.selectedMeasure) && measure === routeInfo.selectedMeasure) {
          // Measure didn't change. Skip updating the point. The user may have selected a gap or
          // branch endpoint with the picker and we don't to move it to to other endpoint.
          return { valid: true }
        }
        if (updateRouteInfo.routeId.length) {
          await QueryMeasureOnRoute(networkDS, network, updateRouteInfo.routeId, updateRouteInfo.fromDate, measure.toString())
            .then((point) => {
              if (isDefined(point)) {
                updateRouteInfo = { ...updateRouteInfo, selectedPoint: point }
                onRouteInfoUpdated(updateRouteInfo)
              }
            })
        } else {
  ...
  const handleRoutePickerChange = () => {
    ...
    if (isPickerActive) {
      ...
      setTimeout(() => {
        jimuMapView.view.popupEnabled = isPopupEnabled && !isMeasurePickerActive
      }, 800)
    }
  }

  const handleMeasurePickerChange = () => {
    const isPickerActive = !isMeasurePickerActive
    setIsMeasurePickerActive(isPickerActive)
    if (isPickerActive) {
      setIsRoutePickerActive(false)
    } else {
      setTimeout(() => {
        jimuMapView.view.popupEnabled = isPopupEnabled && !isRoutePickerActive
      }, 800)
    }
  }
      ...
      {network && (
        <div className='w-100'>
          <Label size="sm" className='w-100 mb-0 pt-3' style={{ width: 100, alignItems: 'center', fontWeight: 500 }} >
            {network.useRouteName ? getI18nMessage('routeNameRequiredLabel') : getI18nMessage('routeIdRequiredLabel')}
          </Label>
          <div ref={searchConRef} className='d-flex w-100'>
            <TextInput
              aria-label={network.useRouteName ? getI18nMessage('routeNameRequiredLabel') : getI18nMessage('routeIdRequiredLabel')}
              type='text'
              className='w-100'
              size='sm'
              allowClear
          ...
          <div className='d-flex w-100'>
            ...
            <RoutePicker
              ...
              hoverGraphic={hoverGraphic}
              isFrom={true}
              onActiveChange={handleMeasurePickerChange}
              onRouteInfoUpdated={handleRouteInfoUpdate }
              onMultiRouteSelect={handleMultiRouteSelect }
            />
          </div>
