...
import { type LrsLayer, LrsLayerType, SearchMethod, type AttributeSets, isDefined, isConflictPreventionEnabled, formatMessage } from 'widgets/shared-code/lrs'
...
export default class Setting extends React.PureComponent<AllWidgetSettingProps<IMConfig>, WidgetSettingState> {
  ...
  getDefaultMethod (): SearchMethod {
    if (this.props.config.defaultMethod) {
      return this.props.config.defaultMethod
    } else {
      this.handleDefaultMethodChanged(this.props.config.defaultMethod)
      return this.props.config.defaultMethod
    }
  }

  handleDefaultMethodChanged = (value: SearchMethod) => {
    this.props.onSettingChange({
      id: this.props.id,
      config: this.props.config.set('defaultMethod', value)
    })
  }
        ...
        <div className="jimu-widget-setting setting-add-point-event__setting-content h-100">
          ...
          {config.eventLayers && config.eventLayers.length !== 0 && (
            <div>
              ...
              >
                <SettingRow flow="wrap" label={this.getI18nMessage('defaultEventSingle')}>
                  <Select
                ...
                <SettingRow flow="wrap" label={this.getI18nMessage('defaultMethod')}>
                  <Select
                    aria-label={this.getI18nMessage('defaultMethod')}
                    className='w-100'
                    size='sm'
                    value={this.getDefaultMethod()}
                    onChange={(e) => { this.handleDefaultMethodChanged(e.target.value) }}>
                      {<option value={SearchMethod.Measure}>{this.getI18nMessage('methodMeasure')}</option>}
                      {/* {<option value={SearchMethod.Coordinate}>{this.getI18nMessage('methodCoordinate')}</option>}
                      {<option value={SearchMethod.LocationOffset}>{this.getI18nMessage('methodLocationOffset')}</option>} */}
                  </Select>
                </SettingRow>
