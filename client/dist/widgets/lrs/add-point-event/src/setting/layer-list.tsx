...
export function LrsLayerList (props: Props) {
  ...
  const [noPointEventLayers, setNoPointEventLayers] = React.useState<boolean>(false)
  ...
  const PopulateLrsLayers = (lrsJson: any) => {
    ...
    } else if (pointEventLayers.length === 0) {
      // No Point event layers found.
      setNoPointEventLayers(true)
      setTimeout(() => {
        setNoPointEventLayers(false)
      }, errorMessageDelay)
          ...
          .then((result: any) => {
            ...
            if (type === 'esriLRSNetworkLayer' ||
              ...
              if (isNetwork && !result.isDerived) {
                ...
                if (resultEventLayers && resultEventLayers.length > 0) {
  ...
  const PopulateAttributeSets = async (url: string) => {
    ...
    const pointAttributeSets: AttributeSets = {
      attributeSet: []
    }
        ...
        if (isDefined(attributeSets)) {
          attributeSets.forEach(element => {
            if (element.isPoint) {
              ...
              pointAttributeSets.attributeSet.push(attributeSet)
            }
          })
        }
        onAttributeSetsChanged(pointAttributeSets)
      })
      .catch((e: any) => {
        return false
      })
          ...
          {noPointEventLayers && (
            <div className="text-break error-tips d-flex align-items-center">
              <Fragment>
                <WarningOutlined color={theme.sys.color.error.main} />
                <div className="display-error-tips">
                  {getI18nMessage('noPointEventLayersTips')}
                </div>
              </Fragment>
            </div>
          )}
