...
import { type LrsLayer, LrsLayerType, type IMConfig, type LocationInfo } from '../../config'
import type Graphic from 'esri/Graphic'
import type Polyline from 'esri/geometry/Polyline'
import type GraphicsLayer from 'esri/layers/GraphicsLayer'
import TimeExtent from 'esri/TimeExtent'
import geometryEngine from 'esri/geometry/geometryEngine'
import { GetUnits, isDefined, getRouteFromEndMeasures, type RouteInfo, getHoverGraphic, getSimplePointGraphic, QueryRouteMeasures } from 'widgets/shared-code/lrs'
...
export function RoutePicker (props: RoutePickerProps) {
  const {
    intl, isReady, active, jimuMapView, hoverGraphic, allDataSources, lrsLayers, config,
  ...
  const getLayerFromDs = (activeFeatureLayer) => {
    ...
    for (let i = 0; i < lrsLayers.length; i++) {
      ...
      if (layer?.serviceId === activeFeatureLayer?.layerId) {
        ...
        if (layerType.toLocaleLowerCase() === LrsLayerType.event.toLocaleLowerCase()) {
          ...
          info.layer = layer
        } else if (layerType.toLocaleLowerCase() === LrsLayerType.network.toLocaleLowerCase()) {
  ...
  const pointerMoveHandler = async (event): Promise<void> => {
    ...
    await Promise.all(allDataSources.map(async (ds) => {
      ...
      if (isDefined(lrsLayer) && lrsLayer.layerType.toLocaleLowerCase() === LrsLayerType.network.toLocaleLowerCase()) {
          ...
          activeFeatureLayer.queryFeatures(query)
            .then(async (response) => {
              if (response && response.features.length > 0 && isProccessingHover.current) {
                ...
                const nearestPoint = geometryEngine.nearestCoordinate(geometry, mapPoint)
                const nearestVertex = geometryEngine.nearestVertex(geometry, mapPoint)
        ...
        await activeFeatureLayer.queryFeatures(query)
          .then(async (response) => {
            await Promise.all(response?.features?.map(async (feature) => {
              ...
              if (layerDetails?.layer?.layerType?.toLocaleLowerCase() === LrsLayerType.network.toLocaleLowerCase()) {
              ...
              } else if (layerDetails?.layer?.layerType?.toLocaleLowerCase() === LrsLayerType.event.toLocaleLowerCase()) {
                eventDataRecords.push({
                  parentNetworkId: layerDetails?.layer?.eventInfo?.parentNetworkId,
                  attributes: attributes,
                  attributeSets: config?.attributeSets,
                  defaultLineAttributeSet: config?.defaultLineAttrbuteSet,
                  defaultPointAttributeSet: config?.defaultPointAttributeSet,
