...
import { CalciteTable, CalciteTableRow, CalciteTableHeader } from 'calcite-components'
...
import { measureFields } from '../../../../search-by-route/src/constants'
...
export function RoutePickerPopup (props: RoutePickerPopupProps) {
  const {
    intl, widgetId, config, outputDS, networkLayers, selectedLocationInfo, eventDetails, jimuMapView, allDataSources, measuresOids,
    isNetworkChange, onRouteInfoUpdated, clearPickedGraphic, handleSelectedNetworkChange
  } = props
  ...
  const getPointFromPolyline = (mapPoint, attributes, measure?) => {
    ...
    const feature = new Graphic({
      geometry: jimuMapView.view.toMap(mapPoint),
  ...
  const handleNetworkChange = React.useCallback((event) => {
    ...
    networkLayers?.forEach(async (routeInfo, index) => {
      if (routeInfo?.id === id) {
            ...
            if ((layer?.layerId === event.eventLayerId) && (
              ...
              fields.forEach((field) => {
                ...
                if (!codeValueLabel) {
                  ...
                  if (fieldType?.toLowerCase() === 'date') val = isDefined(val) ? getDateWithTZOffset(val, selectedNetwork.routes[routeCount].ds).toLocaleDateString() : null
                } else val = codeValueLabel
                if (fieldType?.toLowerCase() === 'date') val = isDefined(val) ? getDateWithTZOffset(val, selectedNetwork.routes[routeCount].ds).toLocaleDateString() : null
