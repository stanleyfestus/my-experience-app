...
export function LrsLayerList (props: Props) {
  const { runningQuery, widgetId, layers = [], mapWidgetIds, portalUrl, theme, setRunningQuery, onLayersAdded, onLayerRemoved, onClearAll, onLayerChanged, onLayerOrderChanged, onMapWidgetedChanged, onAttributeSetsChanged, intl } = props
  ...
  const [noLineEventLayers, setNoLineEventLayers] = React.useState<boolean>(false)
  ...
  const PopulateLrsLayers = (lrsJson: any) => {
    ...
    lrsJson.eventLayers.forEach((item) => {
      if (allLayersAsFeatureLayerDS.findIndex(layer => layer.layerId === item.id.toString()) > -1) {
        ...
        if (item.type === 'esriLRSLinearEventLayer') {
          lineEventLayers.push(item)
        }
      }
    })
    ...
    } else if (lineEventLayers.length === 0) {
      // No Line event layers found.
      setNoLineEventLayers(true)
      setTimeout(() => {
        setNoLineEventLayers(false)
      }, errorMessageDelay)
              ...
              if (isNetwork && !result.isDerived) {
                ...
                if (resultEventLayers.length > 0) {
                  resultEventLayers.forEach(element => {
                    ...
                    //eventsList[element.name] = serviceLayerId
                  })
                }
                ...
                eventInfo.isPointEvent = result.type === 'esriLRSPointEventLayer'
                ...
                const layerGeometryIsPolyline = layerDefinition?.geometryType === 'esriGeometryPolyline'
                ...
                if (layerGeometryIsPolyline) {
                  const allFields = layerDefinition?.fields
                  const shapeLengthField = allFields.find((item) =>
                    item.name === 'Shape__Length'
                  )
                  if (shapeLengthField) {
                    excludedFields.push(shapeLengthField.name)
                  }
                }
  ...
  const PopulateAttributeSets = async (url: string) => {
    ...
    const lineAttributeSets: AttributeSets = {
      attributeSet: []
    }
    ...
    await requestService({
      ...
      .then((result: any) => {
        console.log('attribute sets', result)
        ...
        if (isDefined(attributeSets)) {
          attributeSets.forEach(element => {
            if (!element.isPoint) {
              ...
              lineAttributeSets.attributeSet.push(attributeSet)
            }
          })
        }
        onAttributeSetsChanged(lineAttributeSets)
      })
      .catch((e: any) => {
        return false
      })
          ...
          {noLineEventLayers && (
            <div className="text-break error-tips d-flex align-items-center">
              <Fragment>
                <WarningOutlined color={theme.sys.color.error.main} />
                <div className="display-error-tips">
                  {getI18nMessage('noLineEventLayersTips')}
                </div>
              </Fragment>
            </div>
          )}
