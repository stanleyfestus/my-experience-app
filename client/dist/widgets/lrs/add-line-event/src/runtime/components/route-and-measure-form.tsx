...
export function RouteAndMeasureForm (props: RouteAndMeasureFormProps) {
  ...
  */
  ...
  const validateRouteIdOrName = (routeIdOrName: string, fromDataAction = false): Promise<ValidityResult> => {
    return new Promise(resolve => {
      setTimeout(() => {
        ...
        const isValid = queryRouteIdOrName(routeIdOrName.trim(), network, networkDS, canSpanRoutes, isFrom, lineId)
          .then(async (results) => {
            if (isDefined(results)) {
              ...
              if (routeInfoArray.length > 1 && !fromDataAction) {
                ...
                if (fromDataAction) {
                  updateRouteInfo = routeInfoArray.find((route) => route.fromDate.getTime() === routeInfo.fromDate.getTime())
                }
                if (isFrom) {
            ...
            if (!reset) {
              ...
              if (routeIdOrName.length > 0) {
                return { valid: false, msg: getI18nMessage('routeNotFound') }
              } else {
                updateSearchValue('')
                return { valid: true }
              }
  ...
  const setFromRouteInfoByRouteIdOrName = async (record: __esri.Graphic, routeIdOrName: string): Promise<RouteInfo> => {
    ...
    return await QueryRouteMeasures(networkDS, network, routeEndPoints, routeFromDateValue, routeIdValue)
      .then(async (endpointMeasures) => {
        ...
        if (useStartMeasure) {
          updateRouteInfo.selectedMeasure = routeInfo.fromMeasure
        }
    ...
    return await QueryRouteMeasures(networkDS, network, routeEndPoints, routeFromDateValue, routeIdValue)
      .then(async (endpointMeasures) => {
        ...
        const updateRouteInfo = {
          ...
          toRouteFromDate: canSpanRoutes ? isDefined(routeFromDateValue) ? getDateWithTZOffset(routeFromDateValue, networkDS) : null : routeInfo.toRouteFromDate,
          toRouteToDate: canSpanRoutes ? isDefined(routeToDateValue) ? getDateWithTZOffset(routeToDateValue, networkDS) : null : routeInfo.toRouteToDate,
        ...
        if (useEndMeasure) {
          updateRouteInfo.selectedToMeasure = routeInfo.toRouteToMeasure
        }
        return updateRouteInfo
      })
  }
  ...
  const updateSelectedFromMeasure = (routeInfo: RouteInfo, fromDataAction: boolean): Promise<RouteInfo> => {
    return QueryMeasureOnRoute(networkDS, network, routeInfo.routeId, routeInfo.fromDate, fromDataAction ? routeInfo.selectedMeasure.toString() : fromMeasureInput)
      .then((point) => {
        if (point) {
          const updateRouteInfo = { ...routeInfo, selectedPoint: point, selectedMeasure: parseFloat(fromDataAction ? routeInfo.selectedMeasure.toString() : fromMeasureInput) }
          return updateRouteInfo
        } else {
  ...
  const updateSelectedToMeasure = (routeInfo: RouteInfo, fromDataAction: boolean): Promise<RouteInfo> => {
    return QueryMeasureOnRoute(networkDS, network, routeInfo.toRouteId, routeInfo.toRouteFromDate, fromDataAction ? routeInfo.selectedToMeasure.toString() : toMeasureInput)
      .then((point) => {
        if (point) {
          const updateRouteInfo = { ...routeInfo, selectedToPoint: point, selectedToMeasure: parseFloat(fromDataAction ? routeInfo.selectedToMeasure.toString() : toMeasureInput) }
          return updateRouteInfo
        } else {
          const updateRouteInfo = { ...routeInfo, selectedToPoint: null, selectedToMeasure: NaN }
          return updateRouteInfo
        }
      })
  }
  ...
  const revalidateFields = (validateRoute = true, validateMeasure = true) => {
    if (!revalidateRouteFromDataAction) {
      setTimeout(() => {
        ...
        if (validateMeasure && !hideMeasures) {
  ...
  return (
    <div className='route-and-measure-form d-flex w-100 pt-1 px-3'>
      {network && (
        <div className='w-100'>
          <Label size="sm" className='w-100' style={{ padding: 0, paddingTop: 5, width: 100, alignItems: 'center', fontWeight: 500 }} >
