...
export default class ExportJson extends AbstractDataAction {
  ...
  async onExecute (dataSets: DataRecordSet[], dataLevel: DataLevel): Promise<boolean> {
    ...
    if (dataLevel === DataLevel.DataSource) {
      ...
      rteInfo.toRouteName = isDefined(dataAttributes[routeNameField]) ? String(dataAttributes[routeNameField]) : null
      ...
      rteInfo.toRouteLineOrder = isDefined(dataAttributes[lineOrderField]) ? Number(dataAttributes[lineOrderField]) : null
      ...
      rteInfo.selectedToMeasure = isDefined(dataAttributesNoCase.measure) ? Number(dataAttributesNoCase.measure) : NaN
      ...
      if (!hideMeasures) {
        const geometry = record.getGeometry()
        rteInfo.selectedPoint = null
        rteInfo.selectedToPoint = geometry as Point
      }
