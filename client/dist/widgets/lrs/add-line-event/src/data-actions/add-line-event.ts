...
export default class ExportJson extends AbstractDataAction {
  ...
  async onExecute (dataSets: DataRecordSet[], dataLevel: DataLevel): Promise<boolean> {
    ...
    rteInfo.toRouteName = !twoRouteRecords ? isDefined(dataAttributes[routeNameField]) ? String(dataAttributes[routeNameField]) : null : isDefined(dataAttributes2[routeNameField]) ? String(dataAttributes2[routeNameField]) : null
    rteInfo.lineId = isDefined(dataAttributes[lineIdField]) ? String(dataAttributes[lineIdField]) : null
    rteInfo.toLineId = isDefined(dataAttributes[lineIdField]) ? String(dataAttributes[lineIdField]) : null
    rteInfo.routeLineOrder = isDefined(dataAttributes[lineOrderField]) ? Number(dataAttributes[lineOrderField]) : null
    rteInfo.toRouteLineOrder = !twoRouteRecords ? isDefined(dataAttributes[lineOrderField]) ? Number(dataAttributes[lineOrderField]) : null : isDefined(dataAttributes2[lineOrderField]) ? Number(dataAttributes2[lineOrderField]) : null
    rteInfo.lineName = isDefined(dataAttributes[lineNameField]) ? String(dataAttributes[lineNameField]) : null
    rteInfo.toLineName = isDefined(dataAttributes[lineNameField]) ? String(dataAttributes[lineNameField]) : null
