...
import { type SubtypeLayers, type DynSegFieldInfo, type MeasureRange, type TrackRecord } from '../../../../config'
  ...
  const setPointSymbol = async (forceUpdate?: boolean) => {
    ...
    await symbolUtils.getDisplayedSymbol(graphic, { renderer: eventFS.layer.renderer }).then(async (symbol) => {
      ...
      if (isDefined(symbolLayers)) {
        ...
        symbol = jsonUtils.fromJSON(symbolJson)
