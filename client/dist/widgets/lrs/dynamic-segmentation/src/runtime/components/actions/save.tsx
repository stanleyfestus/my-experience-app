...
import { type MessageProp, type TableEdits } from '../../../config'
...
export function Save (props: SaveProps) {
  ...
  const saveEdits = async () => {
    ...
    let gdbVersion = networkFeatureLayer.getGDBVersion()
    if (!gdbVersion) {
      gdbVersion = ''
    }
    ...
    let lrsUrl = ''
    Promise.all(edits.map(async (edit, index) => {
  ...
  const getLayerEdits = (lrsLayer: LrsLayer, edits: TableEdits, networkName: string): Promise<{ [key: string]: string | number | Date }> => {
    return getLayer(lrsLayer.useDataSource).then(async (featureLayer) => {
