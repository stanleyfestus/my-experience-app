...
export function DynSegCell (props: CellProps) {
  const { intl, featureLayer, record, fieldInfo, subTypeInfo, layerMap, fieldGroups, contingentValues, updateRecord } = props
          ...
          <div className='dyn-seg-cell-content h-100 d-flex' tabIndex={-1}>
            ...
            {!isSelected && (
              <div onDoubleClick={handleDoubleClick} className='d-flex'>
                <Label size="sm" className='text-truncate w-100' style={{ textOverflow: 'ellipse', minWidth: getWidth(), maxWidth: '200px', textWrap: 'nowrap', marginBottom: 0, alignItems: 'center', textAlign: 'left' }} >
                  {getFieldLabel()}
                </Label>
                {errorMsg.length > 0 &&
