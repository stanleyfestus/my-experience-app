...
import { type Track, type TrackRecord } from 'widgets/lrs/dynamic-segmentation/src/config'
  ...
  const getDerivedFieldsValues = (field: __esri.Field) => {
    ...
    if (isDefined(allRecords) && isDefined(currentRecord) && isDefined(networkInfo) && isDefined(field) && allRecords.length) {
      ...
      allRecords.forEach((record) => {
        ...
        if (recordsInTrack.has(objectId)) {
          const recordByOID = recordsInTrack.get(objectId)
  ...
  <div
    ...
    {showDerivedUI
      ...
      css={getCalciteBasicTheme()}>
          <Label
            className='statistic-label text-truncate'
            size='lg'
            centric
            style={{
              fontWeight: 600,
