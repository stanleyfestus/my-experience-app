...
import { type SubtypeLayers, type DynSegFieldInfo, type RouteInfoFromDataAction } from '../../../config'
import { useDynSegRuntimeState } from '../../state'
import { useTheme } from 'jimu-theme'
import { type LrsLayer, type NetworkInfo } from 'widgets/shared-code/lrs'
...
export function DynSegTable (props: DynSegTableProps) {
  const { intl, conflictPreventionEnabled, networkDS, routeId, lrsLayers, currentRouteInfo, networkInfo, featureLayer, records, measureHeaders, fieldInfo, subTypeInfo, layerMap, fieldGroups, contingentValues, handleLockToast } = props
          ...
          if (columnHeaderElm) {
            columnHeaderElm.forEach((elm) => {
              ...
              if (th && th.ariaColIndex === '1') {
                ...
                (th as HTMLElement).style.backgroundColor = theme.sys.color.surface.background
