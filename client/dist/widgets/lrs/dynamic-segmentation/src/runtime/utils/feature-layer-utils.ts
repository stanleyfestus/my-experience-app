...
import { type SubtypeLayers, type AttributeSetParam, type QueryAttributeSetResults } from '../../config'
...
export async function createDynSegFeatureLayer (
  ...
  return await createFeatureLayer(fields, 'polyline', 'hide', jimuMapView, 'dynamic-segmentation')
}
...
export async function cloneFields (
  ...
  return await getLayer(network.useDataSource).then(async (networkLayer) => {
    ...
    attributeSet.forEach(async (set) => {
      ...
      await getLayer(event.useDataSource).then(async (eventLayer) => {
