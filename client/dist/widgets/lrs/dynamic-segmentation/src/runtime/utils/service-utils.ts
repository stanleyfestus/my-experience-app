...
import { type RouteInfoFromDataAction, type LocationParam, type AttributeSetParam, type QueryAttributeSetResults } from '../../config'
...
export async function queryAttributeSets (
  ...
  return requestService({ method: 'POST', url: REST, params: params })
    .then(async (results: QueryAttributeSetResults) => {
      if (!results) {
        return null
      }
      return results
