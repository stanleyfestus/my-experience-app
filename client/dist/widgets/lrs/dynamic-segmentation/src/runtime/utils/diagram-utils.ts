import { isCodedDomain, isDate, isDefined, isNumber, isRangeDomain, type NetworkInfo, type LrsLayer } from 'widgets/shared-code/lrs'
...
import { type FeatureLayerDataSource, loadArcGISJSAPIModules, type CodedValue, loadArcGISJSAPIModule, type ImmutableObject } from 'jimu-core'
...
export function getObjectIdValue (trackRecord: TrackRecord): number {
  ...
  if (isDefined(oidField)) {
    return trackRecord.attributes.get(oidField.featureFieldName) as number ?? NaN
  }
  return NaN
}

async function mergeTracks (trackMap: Map<string, Track>): Promise<Map<string, Track>> {
  const geometryEngine = await loadArcGISJSAPIModule('esri/geometry/geometryEngine')
    ...
    track.records.forEach((record) => {
      if (!currentTrackRecord) {
        ...
        if (canMerge(currentTrackRecord, record)) {
          ...
          currentTrackRecord.geometry = mergeGeometry(geometryEngine, currentTrackRecord.geometry, record.geometry)
        } else {
...
function mergeGeometry (geometryEngine: __esri.geometryEngine, geometry1: __esri.Geometry, geometry2: __esri.Geometry): __esri.Geometry {
  ...
  return geometryEngine.union([geometry1, geometry2])
}
...
export function convertFieldValueToType (field: __esri.Field, featureLayer: __esri.FeatureLayer, value: any, subtypeLayers: SubtypeLayers[]): string {
  const fieldType = getDisplayFieldType(field, null, featureLayer)
  return getValue(fieldType, field, null, value, subtypeLayers, featureLayer)
}

function getValue (fieldType: FieldType, field: __esri.Field, fieldInfo: DynSegFieldInfo, value: any, subtypeLayers: SubtypeLayers[], featureLayer?: __esri.FeatureLayer): string {
  switch (fieldType) {
    case 'subtype':
    ...
    case 'date':
      return isDefined(value) ? new Date(value).toLocaleDateString() : ''
    case 'range':
    case 'number':
    case 'text':
  ...
  fieldInfos.forEach((field) => {
    if (field.eventName === tableName) {
      if (field.isOidField) {
        ...
        attributes.set(field.originalFieldName, trackRecord.attributes.get(field.featureFieldName))
      }
    }
  })
