...
export interface SearchByRouteResultProps {
  ...
  inputDS: DataSource
  isPoint: boolean
  currentNetwork: ImmutableObject<NetworkItem>
...
export function SearchTaskResult (props: SearchByRouteResultProps) {
  ...
  const resultUseOutputDataSource = React.useMemo(() => {
    return Immutable({
      dataSourceId: isPoint ? currentNetwork.outputPointDsId : currentNetwork.outputLineDsId,
      mainDataSourceId: isPoint ? currentNetwork.outputPointDsId : currentNetwork.outputLineDsId
    })
  }, [currentNetwork.outputLineDsId, currentNetwork.outputPointDsId, isPoint])
