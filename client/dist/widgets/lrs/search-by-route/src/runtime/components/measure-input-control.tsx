...
import { type RouteAndMeasureQuery, type NetworkItem, SearchMeasuresType } from '../../config'
import { GetUnits, convertStationToNumber } from 'widgets/shared-code/lrs'
...
export function MeasureInputControl (props: SearchMeasureProps) {
  const { reset, networkItem, isDataSourceReady, searchMeasureBy, routeAndMeasureRequest, intl, onChange } = props
  const getI18nMessage = hooks.useTranslation(defaultMessages)
  const units = GetUnits(networkItem.unitsOfMeasure, intl)
  const [measureOrStationValue, setMeasureOrStationValue] = React.useState(null)
  const [fromMeasureOrStationValue, setFromMeasureOrStationValue] = React.useState(null)
  const [toMeasureOrStationValue, setToMeasureOrStationValue] = React.useState(null)
  const [measureOrStationItems, setMeasureOrStationItems] = React.useState<string[]>(new Array<string>(1).fill(''))

  React.useEffect(() => {
    setMeasureOrStationValue(null)
    setFromMeasureOrStationValue(null)
    setToMeasureOrStationValue(null)
